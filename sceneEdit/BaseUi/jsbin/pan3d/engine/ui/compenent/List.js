    ...
    (function (me) {
        var List = /** @class */ (function (_super) {
            ...
            function List() {
                ...
                _this.addEventListener(me.InteractiveEvent.Down, _this.onDown, _this);
                _this.addEventListener(me.InteractiveEvent.Up, _this.onListUp, _this);
                return _this;
            }
            List.prototype.applyAbsolutePoint = function () {
                if (this.parent) {
                    this.absoluteX = (this._x * this.scale + this._contentX) * me.UIData.Scale + this.parent.x;
                    this.absoluteY = (this._y * this.scale + this._contentY) * me.UIData.Scale + this.parent.y;
                    this.absoluteWidth = this.width * me.UIData.Scale;
                    this.absoluteHeight = this.height * me.UIData.Scale;
            ...
            List.prototype.testPoint = function ($x, $y) {
                if ($x > this.absoluteX
                    && $x < (this.absoluteX + this._showWidth * me.UIData.Scale)
                    && $y > this.absoluteY
                    && $y < (this.absoluteY + this._showHeight * me.UIData.Scale)) {
                    return true;
                }
                else {
                    return false;
            ...
            List.prototype.onDown = function (event) {
                ...
                if (this._needScoller) {
                    me.Scene_data.uiStage.addEventListener(me.InteractiveEvent.Move, this.onMove, this);
                    me.Scene_data.uiStage.addEventListener(me.InteractiveEvent.Up, this.onUp, this);
                }
            };
            List.prototype.onListUp = function (event) {
                if (this.uiRender.mask) {
                    ...
                    if (Math.abs(this.contentY - this.lastcontentY) < 1) {
                        var ty = (event.y - this.absoluteY + this.contentY) / me.UIData.Scale;
            ...
            List.prototype.testItemClick = function ($xPos, $ypos) {
                ...
                var itemH = this._itemHeight * me.UIData.Scale;
            ...
            List.prototype.onUp = function (event) {
                me.Scene_data.uiStage.removeEventListener(me.InteractiveEvent.Move, this.onMove, this);
                me.Scene_data.uiStage.removeEventListener(me.InteractiveEvent.Up, this.onUp, this);
            };
            return List;
        }(me.UICompenent));
        me.List = List;
        var ListAtlas = /** @class */ (function (_super) {
            ...
            ListAtlas.prototype.setData = function ($width, $height, itemWidth, itemHeight, itemNum) {
                this.ctx = me.UIManager.getInstance().getContext2D($width, $height, false);
                //UIManager.getInstance().showCanvas(0,0);
                //this.ctx.fillStyle = "#6600ff";
                // this.ctx.fillRect(0, 0, $width, $height);
                this.textureRes = me.TextureManager.getInstance().getCanvasTexture(this.ctx);
            ...
            ListItemRender.prototype.click = function (xpos, ypos) {
                if (this._listItemData.clickFun) {
                    ...
                    if (this._listItemData.clickRect) {
                        if (!this._listItemData.clickRect.isHitByPoint(xpos / me.UIData.Scale, ypos / me.UIData.Scale)) {
                            return;
                        }
                    }
        ...
        me.ListItemRender = ListItemRender;
