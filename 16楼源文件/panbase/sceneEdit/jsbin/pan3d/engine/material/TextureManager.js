        ...
        var TextureManager = /** @class */ (function (_super) {
            ...
            TextureManager.prototype.getTexture = function ($url, $fun, $wrapType, $info, $filteType, $mipmapType) {
                ...
                if (this._resDic[$url]) {
                    ...
                    me.LoadManager.getInstance().load($url, me.LoadManager.IMG_TYPE, function ($img, _info) {
            ...
            TextureManager.prototype.getImageData = function ($url, $fun) {
                me.LoadManager.getInstance().load($url, me.LoadManager.IMG_TYPE, function ($img) {
                    var ctx = me.UIManager.getInstance().getContext2D($img.width, $img.height, false);
            ...
            TextureManager.prototype.addImgRes = function ($url, $img) {
                this._resDic[$url] = $img;
                var texture = me.Scene_data.context3D.getTexture($img);
                var textres = new me.TextureRes();
            ...
            TextureManager.prototype.getCanvasTexture = function (ctx) {
                var tres = new me.TextureRes;
                var texture = me.Scene_data.context3D.getTexture(ctx.canvas, 0, 0);
                tres.texture = texture;
                return tres;
            };
            TextureManager.prototype.getImageDataTexture = function (imgdata) {
                var texture = me.Scene_data.context3D.getTexture(imgdata, 0, 0);
                return texture;
            };
            TextureManager.prototype.getTextureRes = function ($img) {
                var tres = new me.TextureRes;
                var texture = me.Scene_data.context3D.getTexture($img, 0, 0);
                tres.texture = texture;
                return tres;
            };
            TextureManager.prototype.updateTexture = function ($texture, $offsetx, $offsety, ctx) {
                me.Scene_data.context3D.updateTexture($texture, $offsetx, $offsety, ctx.canvas);
            };
            ...
            TextureManager.prototype.loadTextureCom = function ($img, _info) {
                var texture = me.Scene_data.context3D.getTexture($img, _info.wrap, _info.filter, _info.mipmap);
                var textres = new me.TextureRes();
                textres.texture = texture;
                textres.width = $img.width;
                textres.height = $img.height;
            ...
            TextureManager.prototype.initDefaultLightMapTexture = function () {
                ...
                this.defaultLightMap = me.Scene_data.context3D.getTexture(canvas);
            };
        ...
        var CubemapLoad = /** @class */ (function () {
            ...
            CubemapLoad.prototype.loadCube = function ($url, $fun) {
                ...
                for (var i = 0; i < 6; i++) {
                    ...
                    console.log(itemUrl);
                    me.LoadManager.getInstance().load(itemUrl, me.LoadManager.IMG_TYPE, function ($img, $info) { _this.loadCom($img, $info); }, { "id": i });
                }
            };
