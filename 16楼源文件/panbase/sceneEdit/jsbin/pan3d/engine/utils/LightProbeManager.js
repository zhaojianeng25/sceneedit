    ...
    (function (me) {
        var LightProbeManager = /** @class */ (function () {
            function LightProbeManager() {
                ...
                for (var i = 0; i < 9; i++) {
                    this._defaultVec.push(new me.Vector3D(ary[i], ary[i], ary[i]));
                }
            }
            ...
            LightProbeManager.prototype.getResultData = function (ary, x, z, y, bNum, $pos) {
                ...
                for (i = 0; i < 9; i++) {
                    ...
                    for (var j = 0; j < posAry.length; j++) {
                        var tempV3d = new me.Vector3D(posAry[j].vecNum[i].x, posAry[j].vecNum[i].y, posAry[j].vecNum[i].z);
        ...
        me.LightProbeManager = LightProbeManager;
        var PosItem = /** @class */ (function () {
            function PosItem(basePos, centerPos) {
                this.pos = new me.Vector3D(basePos.x, basePos.y, basePos.z);
                this.vecNum = basePos.resultSHVec;
                this.dis = me.Vector3D.distance(this.pos, centerPos);
            }
