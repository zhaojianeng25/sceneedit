    ...
    (function (me) {
        var ParticleBallData = /** @class */ (function (_super) {
            ...
            function ParticleBallData() {
                ...
                _this._toscale = 0.00;
                _this._shootAngly = new me.Vector3D(1, 0, 0);
                _this._shootSpeed = 0;
                ...
                _this._addforce = new me.Vector3D(0, 0, 0);
                _this._lixinForce = new me.Vector3D(0, 0, 0);
                _this._waveform = new me.Vector3D(0, 0, 0, 0);
                _this._round = new me.Vector3D();
                _this._is3Dlizi = false;
                _this._speed = 1;
                _this._isLoop = false;
                _this._basePositon = new me.Vector3D(0, 0, 0);
            ...
            ParticleBallData.prototype.getParticle = function () {
                return new me.Display3DBallPartilce;
            };
            ParticleBallData.prototype.setAllByteInfo = function ($byte) {
                ...
                this._particleRandomScale = new me.Vector3D();
                ...
                if (this._is3Dlizi) {
                    ...
                    this._allRotationMatrix = new me.Matrix3D();
                }
                this.initVcData();
            };
            ...
            ParticleBallData.prototype.uploadGpu = function () {
                this.objData = new me.ParticleBallGpuData();
                this.initBaseData();
                this.initBasePos();
                this.initSpeed();
            ...
            ParticleBallData.prototype.makeRectangleData = function (verterList, uvAry, width, height, offsetX, offsetY, isUV, isU, isV, animLine, animRow, indexID) {
                ...
                var ary = new Array;
                ary.push(new me.Vector2D(0, 0));
                ary.push(new me.Vector2D(0, 1 / animRow));
                ary.push(new me.Vector2D(1 / animLine, 1 / animRow));
                ary.push(new me.Vector2D(1 / animLine, 0));
                if (isU) {
            ...
            ParticleBallData.prototype.initBasePos = function () {
                ...
                for (var i = 0; i < this._totalNum; i++) {
                    ...
                    var ma;
                    if (this._isRandom) {
                        var roundv3d = new me.Vector3D(this._round.x * this._round.w, this._round.y * this._round.w, this._round.z * this._round.w);
                        if (this._isEven) { //圆柱
                            if (this._closeSurface) { //紧贴表面
                                v3d = new me.Vector3D(0, 0, roundv3d.z);
                                ma = new me.Matrix3D;
                                ma.appendRotation(Math.random() * 360, me.Vector3D.Y_AXIS);
                                v3d = ma.transformVector(v3d);
                                v3d.y = roundv3d.y * Math.random() * 2 - roundv3d.y;
                            }
                            else {
                                v3d = new me.Vector3D(0, 0, roundv3d.z * Math.random() * 2 - roundv3d.z);
                                ma = new me.Matrix3D;
                                ma.appendRotation(Math.random() * 360, me.Vector3D.Y_AXIS);
                                v3d = ma.transformVector(v3d);
                                v3d.y = roundv3d.y * Math.random() * 2 - roundv3d.y;
                            ...
                            if (this._closeSurface) { //只有xyz相等时候才能紧贴表面
                                ...
                                ma = new me.Matrix3D;
                                if (this._halfCircle) {
                                    ma.appendRotation(-Math.random() * 180, me.Vector3D.X_AXIS);
                                }
                                else {
                                    ma.appendRotation(Math.random() * 360, me.Vector3D.X_AXIS);
                                }
                                ma.appendRotation(Math.random() * 360, me.Vector3D.Y_AXIS);
                                v3d = ma.transformVector(v3d);
                            }
                                ...
                                if (this._halfCircle) {
                                    v3d = new me.Vector3D(roundv3d.x * Math.random() * 2 - roundv3d.x, roundv3d.y * Math.random(), roundv3d.z * Math.random() * 2 - roundv3d.z);
                                }
                                else {
                                    v3d = new me.Vector3D(roundv3d.x * Math.random() * 2 - roundv3d.x, roundv3d.y * Math.random() * 2 - roundv3d.y, roundv3d.z * Math.random() * 2 - roundv3d.z);
                                }
                            }
                        }
                    }
            ...
            ParticleBallData.prototype.initSpeed = function () {
                ...
                for (var i = 0; i < this._totalNum; i++) {
                    ...
                    if (this._shootAngly.x != 0 || this._shootAngly.y != 0 || this._shootAngly.z != 0) { //锥形速度
                        ...
                        v3d = new me.Vector3D(Math.sin(a) * r, Math.cos(a) * r, 1);
                        var ma = new me.Matrix3D(); //moveMatrix3D();
                        ma.fromVtoV(new me.Vector3D(0, 0.0101, 0.99994), new me.Vector3D(this._shootAngly.x, this._shootAngly.y, this._shootAngly.z));
                        v3d = ma.transformVector(v3d);
                        if (isNaN(v3d.x)) {
                    ...
                    if (this._lixinForce.x != 0 || this._lixinForce.y != 0 || this._lixinForce.z != 0) {
                        v3d = new me.Vector3D(Math.random() > 0.5 ? -this._lixinForce.x : this._lixinForce.x, Math.random() > 0.5 ? -this._lixinForce.y : this._lixinForce.y, Math.random() > 0.5 ? -this._lixinForce.z : this._lixinForce.z);
                        v3d.normalize();
                        resultv3d = resultv3d.add(v3d);
                    }
                    if (this._islixinAngly) {
                        if (this._isEven) {
                            v3d = new me.Vector3D(this.objBallData.basePos[i * 16], 0, this.objBallData.basePos[i * 16 + 2]);
                        }
                        else {
                            v3d = new me.Vector3D(this.objBallData.basePos[i * 16], this.objBallData.basePos[i * 16 + 1], this.objBallData.basePos[i * 16 + 2]);
                        }
                        v3d.normalize();
                        resultv3d = resultv3d.add(v3d);
                    }
            ...
            ParticleBallData.prototype.initBaseColor = function () {
                var imgData = me.ColorTransition.getInstance().getImageData(this._textureRandomColorInfo);
                var colorNum = imgData.data;
                var colors = new Array;
                for (var i = 0; i < this._totalNum; i++) {
                    var index = float2int(128 * Math.random()) * 4;
                    var ranColor = new me.Vector3D(colorNum[index], colorNum[index + 1], colorNum[index + 2], colorNum[index + 3]);
            ...
            ParticleBallData.prototype.compressVertex = function () {
                ...
                this.objBallData.vertexBuffer = me.Scene_data.context3D.uploadBuff3D(ary);
                this.objBallData.indexBuffer = me.Scene_data.context3D.uploadIndexBuff3D(this.objBallData.indexs);
                this.objBallData.treNum = this.objBallData.indexs.length;
                ////console.log(ary.length);
            };
            ParticleBallData.prototype.setFloat32Vec = function (key, ary) {
                var idxary = me.Display3DBallShader.shader_vec4[key];
            ...
            ParticleBallData.prototype.setFloat32Mat = function (key, ary) {
                var idx = me.Display3DBallShader.shader_mat4[key] * 16;
                this.vcmatData.set(ary, idx);
            };
            ParticleBallData.prototype.initVcData = function () {
                this.vcmatData = new Float32Array(me.Display3DBallShader.getVcSize() * 16);
            ...
            ParticleBallData.prototype.regShader = function () {
                ...
                this.materialParam.shader = me.ProgrmaManager.getInstance().getMaterialProgram(me.Display3DBallShader.Display3D_Ball_Shader, me.Display3DBallShader, this.materialParam.material, shaderParameAry);
        ...
        me.ParticleBallData = ParticleBallData;
