    ...
    (function (me) {
        var GuidObjectTable = /** @class */ (function (_super) {
            ...
            function GuidObjectTable() {
                ...
                _this._newEvent = new me.NetEventDispatcher(me.NetEventDispatcher.KEY_TYPE_STRING);
                _this._delEvent = new me.NetEventDispatcher(me.NetEventDispatcher.KEY_TYPE_STRING);
                _this._indexer = new me.StringIndexer();
                return _this;
            }
            ...
            GuidObjectTable.prototype.ApplyBlock = function (bytes) {
                while (bytes.bytesAvailable) {
                    ...
                    if (flags & me.SyncEvent.OBJ_OPT_U_GUID) {
                    ...
                    var evFilter;
                    //如果是删除则触发事件
                    if (flags & me.SyncEvent.OBJ_OPT_DELETE) {
                    ...
                    var applyNew = (flags & me.SyncEvent.OBJ_OPT_NEW) == 0;
                    if (!cur_obj) {
                        if (flags & me.SyncEvent.OBJ_OPT_NEW) {
                    ...
                    if (flags & me.SyncEvent.OBJ_OPT_UPDATE) {
            ...
            GuidObjectTable.prototype.newPacket = function (optCode) {
                ...
                if (this._packet_pool.length == 0) {
                    pkt = new me.Pan3dByteArray;
                    pkt.endian = me.Endian.LITTLE_ENDIAN;
                }
                else {
            ...
            GuidObjectTable.prototype.clearObjs = function () {
                for (var key in this._objs) {
                    if (this._objs[key] instanceof me.GuidObject) {
            ...
            GuidObjectTable.applyBlock_tmp_obj = new me.GuidObject;
            return GuidObjectTable;
        }(me.SyncEvent));
        me.GuidObjectTable = GuidObjectTable;
