...
(function (Pan3d) {
    ...
    (function (me) {
        var UIConatiner = /** @class */ (function () {
            ...
            */
            UIConatiner.prototype.addEvntBut = function ($name, $uiRender) {
                ...
                $temp.addEventListener(me.InteractiveEvent.Down, this.butClik, this);
                return $temp;
            };
            UIConatiner.prototype.addEvntButUp = function ($name, $uiRender) {
                var $temp = this.addChild($uiRender.getComponent($name));
                $temp.addEventListener(me.InteractiveEvent.Up, this.butClik, this);
                return $temp;
            };
            /*
            *移除事件UI
            */
            UIConatiner.prototype.removeEvntBut = function ($ui) {
                $ui.removeEventListener(me.InteractiveEvent.Down, this.butClik, this);
                this.removeChild($ui);
            };
            ...
            */
            ...
            UIConatiner.prototype.addRenderAt = function ($uiRender, $idx) {
                ...
                if (this.hasStage) {
                    me.UIManager.getInstance().addUI($uiRender);
                }
            };
            UIConatiner.prototype.removeRender = function ($uiRender) {
                ...
                if (this.hasStage) {
                    me.UIManager.getInstance().removeUI($uiRender);
                }
            };
            Object.defineProperty(UIConatiner.prototype, "panelScale", {
                get: function () {
                    return this.uiScale * me.UIData.Scale;
                },
                enumerable: true,
                configurable: true
            });
            UIConatiner.prototype.resize = function () {
                if (this._xType == 0) {
                    ...
                    this._x = me.Scene_data.stageWidth - this._right * this.panelScale - this.width * this.panelScale;
                }
                else if (this._xType == 2) {
                    this._x = this._center * this.panelScale + me.Scene_data.stageWidth / 2 - this.width * this.panelScale / 2;
                }
                if (this._yType == 0) {
                    this._y = this._top * this.panelScale;
                }
                else if (this._yType == 1) {
                    this._y = me.Scene_data.stageHeight - this._bottom * this.panelScale - this.height * this.panelScale;
                }
                else if (this._yType == 2) {
                    this._y = this._middle * this.panelScale + me.Scene_data.stageHeight / 2 - this.height * this.panelScale / 2;
                }
                this.applyChild();
            ...
            UIConatiner.prototype.makeBaseUiatlas = function (w, h) {
                var $uiAtlas = new me.UIAtlas();
                $uiAtlas.configData = [];
                var kkwA = Math.pow(2, Math.ceil(Math.log(w) / Math.log(2)));
                var kkhB = Math.pow(2, Math.ceil(Math.log(h) / Math.log(2)));
                $uiAtlas.ctx = me.UIManager.getInstance().getContext2D(kkwA, kkhB, false);
                $uiAtlas.textureRes = me.TextureManager.getInstance().getCanvasTexture($uiAtlas.ctx);
                $uiAtlas.configData.push($uiAtlas.getObject("temp_ui", 0, 0, w, h, kkwA, kkhB));
                return $uiAtlas;
            };
            return UIConatiner;
        }());
        me.UIConatiner = UIConatiner;
        ...
        me.Dis2DUIContianerBase = Dis2DUIContianerBase;
