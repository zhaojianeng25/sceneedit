        ...
        var SceneChar = /** @class */ (function (_super) {
            ...
            SceneChar.prototype.play = function ($action, $completeState, needFollow) {
                ...
                if (this.isSinging) {
                    ...
                    if ($action == me.CharAction.WALK || $action == me.CharAction.STANAD) {
                        return true;
                    }
                }
                if (this.isMount) {
                    this.mountChar.visible = Boolean($action != me.CharAction.JUMP);
                    if ($action == me.CharAction.STANAD) {
                        _super.prototype.play.call(this, me.CharAction.STAND_MOUNT);
                    }
                    else if ($action == me.CharAction.WALK) {
                        _super.prototype.play.call(this, me.CharAction.WALK_MOUNT);
                    }
                    else {
                        if (this.mountChar.visible) {
                            ...
                            _super.prototype.play.call(this, me.CharAction.JUMP);
                        }
                    }
            ...
            SceneChar.prototype.showName = function ($color) {
                ...
                if (!this._charNameVo) {
                    this._charNameVo = me.BloodManager.getInstance().getCharNameMeshVo($colorName);
                }
                else {
            ...
            SceneChar.prototype.showBlood = function ($colorType) {
                ...
                if (!this._charBloodVo) {
                    this._charBloodVo = me.BloodManager.getInstance().getBloodLineMeshVo();
            ...
            Object.defineProperty(SceneChar.prototype, "walkPath", {
                set: function ($wp) {
                    ...
                    if (this.curentAction == me.CharAction.STANAD || this.curentAction == me.CharAction.STAND_MOUNT) {
            ...
            */
            ...
            SceneChar.prototype.fixAstartData = function (pos) {
                if (this._walkPath) {
                    for (var i = 0; i < this._walkPath.length; i++) {
                        ...
                        this._walkPath[i].y = me.AstarUtil.getHeightByPos(this._walkPath[i]);
                    }
                }
                this.px += pos.x;
            ...
            SceneChar.prototype.applyWalk = function ($item) {
                if ($item && $item.length == 2) {
                    ...
                    if ($item[0].x == $item[1].x && $item[0].y == $item[1].y) {
                        this._speedDirect = null;
                        this._walkPath = null;
                        if (this.curentAction == me.CharAction.WALK) {
                            this.play(me.CharAction.STANAD);
                        }
                        var $k = me.AstarUtil.getWorldPosByStart2D($item[0]);
                        this.px = $k.x;
                        this.pz = $k.z;
                        return;
                    }
                }
                this.walkPath = me.AstarUtil.Path2dTo3d($item);
            };
            Object.defineProperty(SceneChar.prototype, "moveToPos2D", {
                set: function ($v2d) {
                    ...
                    var pos = me.AstarUtil.getWorldPosByStart2D($v2d);
                    this.px = pos.x;
                    this.pz = pos.z;
                    this.refreshY();
                },
            ...
            SceneChar.prototype.walkAstar = function (t) {
                ...
                var distance = me.Vector3D.distance(new me.Vector3D(this.px, 0, this.pz), this._astatTopos);
                if (distance > 5) {
            ...
            SceneChar.prototype.setAstarNrmAndRotation = function () {
                if (this._astatTopos) {
                    ...
                    if (me.Vector3D.distance(this.getCurrentPos(), this._astatTopos) > 10) {
            ...
            SceneChar.prototype.getCurrentPos = function () {
                return new me.Vector3D(this.px, this.py, this.pz);
            };
            SceneChar.prototype.getAstarPos = function () {
                return me.AstarUtil.getGrapIndexByPos(this.getCurrentPos());
            };
            SceneChar.prototype.changeAction = function ($action) {
                if (this.unit && this.unit.isMain) {
                    switch ($action) {
                        case me.CharAction.ATTACK_01:
                            this.play(me.CharAction.ATTACK_010, 2);
                            break;
                        case me.CharAction.ATTACK_02:
                            this.play(me.CharAction.ATTACK_020, 2);
                            break;
                        default:
            ...
            SceneChar.prototype.playSkill = function ($skill) {
                this._walkPath = null;
                me.SkillManager.getInstance().playSkill($skill);
                this.skillVo = $skill;
            };
            ...
            SceneChar.prototype.removeStage = function () {
                ...
                if (this.mountChar) {
                    me.SceneManager.getInstance().removeMovieDisplay(this.mountChar);
                }
                if (this._wingDisplay) {
                    me.SceneManager.getInstance().removeMovieDisplay(this._wingDisplay);
                }
            };
            SceneChar.prototype.addStage = function () {
                ...
                if (this.mountChar) {
                    me.SceneManager.getInstance().addMovieDisplay(this.mountChar);
                }
                if (this._wingDisplay) {
                    me.SceneManager.getInstance().addMovieDisplay(this._wingDisplay);
                }
            };
            SceneChar.prototype.math_distance = function ($other) {
                return me.MathClass.math_distance(this.px, this.pz, $other.x, $other.z);
            };
            ...
            SceneChar.prototype.update = function () {
                ...
                if (this._showHitBox) {
                    if (!this.lineSprite) {
                        me.ProgrmaManager.getInstance().registe(me.LineDisplayShader.LineShader, new me.LineDisplayShader);
                        this.lineSprite = new me.LineDisplaySprite();
            ...
            SceneChar.prototype.mouseClik = function ($lineA, $lineB) {
                var $pos = me.Scene_data.cam3D.cameraMatrix.transformVector(this.getCurrentPos());
                if ($pos.z < me.Scene_data.cam3D.distance / 3) { //在Z后面
                    return null;
                }
                var hitVec2 = me.MathUtil.math3DWorldtoDisplay2DPos($lineB);
                if (this._skinMesh) {
                    ...
                    for (var j = 0; j < this._skinMesh.hitPosItem.length; j++) {
                        ...
                        this.hitBox2DItem.push(me.MathUtil.math3DWorldtoDisplay2DPos(temppp));
                    }
                    for (var i = 0; i < this.triIndex.length / 3; i++) {
                        me.TestTriangle.baseTri.p1 = this.hitBox2DItem[this.triIndex[i * 3 + 0]];
                        me.TestTriangle.baseTri.p2 = this.hitBox2DItem[this.triIndex[i * 3 + 1]];
                        me.TestTriangle.baseTri.p3 = this.hitBox2DItem[this.triIndex[i * 3 + 2]];
                        if (me.TestTriangle.baseTri.checkPointIn(hitVec2)) {
                            return true;
                        }
                    }
                }
                else {
                    if (me.Vector2D.distance(hitVec2, me.MathUtil.math3DWorldtoDisplay2DPos(this.posMatrix.position)) < 20) {
                        return true;
                    }
                }
                return false;
