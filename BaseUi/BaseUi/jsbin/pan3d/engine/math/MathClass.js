    ...
    (function (me) {
        var MathClass = /** @class */ (function () {
            ...
            MathClass.getCamView = function (_Cam, _focus_3d) {
                ...
                $m.appendRotation(-_focus_3d.rotationX, me.Vector3D.X_AXIS);
                $m.appendRotation(-_focus_3d.rotationY, me.Vector3D.Y_AXIS);
                $m.appendTranslation(_focus_3d.x, _focus_3d.y, _focus_3d.z);
                var $p = $m.transformVector(new me.Vector3D(0, 0, -_Cam.distance));
                _Cam.x = $p.x;
                _Cam.y = $p.y;
                _Cam.z = $p.z;
            ...
            MathClass.updateVp = function () {
                me.Scene_data.vpMatrix.identity();
                me.Scene_data.vpMatrix.prepend(me.Scene_data.viewMatrx3D);
                me.Scene_data.vpMatrix.prepend(me.Scene_data.cam3D.cameraMatrix);
            };
            MathClass.MathCam = function (_Cam) {
                var camera3dMatrix = new me.Matrix3D();
                camera3dMatrix.prependRotation(_Cam.rotationX, me.Vector3D.X_AXIS);
                camera3dMatrix.prependRotation(_Cam.rotationY, me.Vector3D.Y_AXIS);
            ...
            MathClass.GetViewHitBoxData = function ($far) {
                var cameraMatrixInvert = me.Scene_data.cam3D.cameraMatrix.clone();
                cameraMatrixInvert.invert();
                var sx = me.Scene_data.viewMatrx3D.m[0];
                var sy = me.Scene_data.viewMatrx3D.m[5];
                if (this.viewBoxVecItem) {
                    if (this.lastViewScale.x != sx || this.lastViewScale.y != sy) {
                        this.viewBoxVecItem[0] = (new me.Vector3D(-$far / sx, -$far / sy, $far));
                        this.viewBoxVecItem[1] = (new me.Vector3D(-$far / sx, +$far / sy, $far));
                        this.viewBoxVecItem[2] = (new me.Vector3D(+$far / sx, -$far / sy, $far));
                        this.viewBoxVecItem[3] = (new me.Vector3D(+$far / sx, +$far / sy, $far));
                        this.viewBoxVecItem[4] = (new me.Vector3D(0, 0, 0));
                    }
                }
                else {
                    this.lastViewScale = new me.Vector2D(sx, sy);
                    this.viewBoxVecItem = new Array;
                    this.viewBoxVecItem.push(new me.Vector3D(-$far / sx, -$far / sy, $far));
                    this.viewBoxVecItem.push(new me.Vector3D(-$far / sx, +$far / sy, $far));
                    this.viewBoxVecItem.push(new me.Vector3D(+$far / sx, -$far / sy, $far));
                    this.viewBoxVecItem.push(new me.Vector3D(+$far / sx, +$far / sy, $far));
                    this.viewBoxVecItem.push(new me.Vector3D(0, 0, 0));
                }
            };
            MathClass.GetViewHitBoxDataCopy = function ($dis) {
                if (!this.viewBoxVecItem) {
                    this.viewBoxVecItem = new Array;
                    this.viewBoxVecItem.push(new me.Vector3D());
                    this.viewBoxVecItem.push(new me.Vector3D());
                    this.viewBoxVecItem.push(new me.Vector3D());
                    this.viewBoxVecItem.push(new me.Vector3D());
                }
                var $disNum = $dis / (me.Scene_data.sceneViewHW / 2);
                ...
                this.viewBoxVecItem[0] = this.gettempPos(new me.Vector3D(-fw, -fh, $far), m);
                this.viewBoxVecItem[1] = this.gettempPos(new me.Vector3D(+fw, -fh, $far), m);
                this.viewBoxVecItem[2] = this.gettempPos(new me.Vector3D(+fw, +fh, $far), m);
                this.viewBoxVecItem[3] = this.gettempPos(new me.Vector3D(-fw, +fh, $far), m);
            };
            ...
            MathClass.mathDisplay2Dto3DWorldPos = function ($stage3DVO, $point, $depht) {
                ...
                var cameraMatrixInvert = me.Scene_data.cam3D.cameraMatrix.clone();
                var viewMatrx3DInvert = me.Scene_data.viewMatrx3D.clone();
                cameraMatrixInvert.invert();
                viewMatrx3DInvert.invert();
                var a = new me.Vector3D();
