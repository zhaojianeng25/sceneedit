    ...
    (function (me) {
        var SkillManager = /** @class */ (function (_super) {
            ...
            SkillManager.prototype.update = function () {
                var _tempTime = me.TimeUtil.getTimer();
            ...
            SkillManager.prototype.preLoadSkill = function ($url) {
                ...
                me.ResManager.getInstance().loadSkillRes(me.Scene_data.fileRoot + $url, function ($skillRes) {
                    var skillData = new me.SkillData();
            ...
            SkillManager.prototype.gc = function () {
                ...
                for (var key in this._dic) {
                    ...
                    if (rc.useNum <= 0) {
                        rc.idleTime++;
                        if (rc.idleTime >= me.ResCount.GCTime && rc.testDestory()) {
                            //console.log("清理 -" + key);
                            rc.destory();
                ...
                for (var key in this._skillDic) {
                    ...
                    for (var i = ary.length - 1; i >= 0; i--) {
                        if (ary[i].isDeath && ary[i].useNum <= 0) {
                            ary[i].idleTime++;
                            if (ary[i].idleTime >= me.ResCount.GCTime) {
                                if (!ary[i].src) {
                                    ary[i].destory();
                                    ary.splice(i, 1);
                                }
        ...
        var ShockUtil = /** @class */ (function () {
            ...
            ShockUtil.prototype.update = function ($dtime) {
                ...
                if (this.ctime > this.time) {
                    me.TimeUtil.removeFrameTick(this.upFun);
                    me.Scene_data.cam3D.offset.setTo(0, 0, 0);
                    return;
                }
                var ranX = (Math.random() - 0.5) * this.amp;
                var ranY = (Math.random() - 0.5) * this.amp;
                var ranZ = (Math.random() - 0.5) * this.amp;
                me.Scene_data.cam3D.offset.setTo(ranX, ranY, ranZ);
            };
            ShockUtil.prototype.shock = function (time, amp) {
                this.time = time;
                this.ctime = 0;
                this.amp = amp;
                me.TimeUtil.addFrameTick(this.upFun);
            };
            return ShockUtil;
        }());
        me.ShockUtil = ShockUtil;
