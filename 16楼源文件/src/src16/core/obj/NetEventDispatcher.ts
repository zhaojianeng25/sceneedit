module core.obj {
  	// 回调结构体
    export class HanlderStruct implements IPoolsObject{
        /** 执行域(this)。*/
        caller: any;
        /** 处理方法。*/
        method: Function;
	...
	*/	
	export class NetEventDispatcher {
		...
		private DispatchIndex(param:Object):void{		
			...
			if(this._event_key_type == NetEventDispatcher.KEY_TYPE_STRING
				...
				for(i in this._event_index){
					let struct =  this._event_callback[this._event_index[i]];
		 ...
		 */		
		public AddListenInt(key:number, caller: any,f:Function):void{
			if(this._event_key_type != NetEventDispatcher.KEY_TYPE_INT)
		...
		public AddListenString(key:string, caller: any,f:Function):void{
			if(this._event_key_type != NetEventDispatcher.KEY_TYPE_STRING)
		...
		public AddListenIntMask(key:UpdateMask, caller: any,f:Function):void{
			if(this._event_key_type != NetEventDispatcher.KEY_TYPE_INT_MASK)
		 ...
		 */
		public removeListenerInt(key:number, caller: any,f:Function = null):void{
			if(this._event_key_type != NetEventDispatcher.KEY_TYPE_INT)
			...
			for(var i:number = 0; i < this._event_id_int.length;){
				if(this._event_id_int[i] == key){
					let struct = this._event_callback[i]
		 ...
		 * @param f 回调函数闭包,可以支持一个参数(Object)，如果f为空，则移除所有
		 */		
		public removeListenerString(key:string, caller: any,f:Function = null):void{
			if(this._event_key_type != NetEventDispatcher.KEY_TYPE_STRING)
		 ...
		 * @param f
		 */		
		public removeListenerUpdateMask(key:UpdateMask, caller: any,f:Function = null):void{
			if(this._event_key_type != NetEventDispatcher.KEY_TYPE_INT_MASK)
		 ...
		 */		
		public AddCallback(caller: any, f:Function):number{
			if(this._event_key_type == NetEventDispatcher.KEY_TYPE_STRING)
