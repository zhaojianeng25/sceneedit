    ...
    (function (me) {
        var UIAtlas = /** @class */ (function () {
            ...
            UIAtlas.prototype.setInfo = function (configUrl, imgUrl, $fun, useImgUrl) {
                ...
                me.LoadManager.getInstance().load(me.Scene_data.fileuiRoot + configUrl, me.LoadManager.XML_TYPE, function ($str) {
            ...
            UIAtlas.prototype.loadImgUrl = function (imgUrl, $fun) {
                var _this = this;
                me.TextureManager.getInstance().getTexture(me.Scene_data.fileuiRoot + imgUrl, function ($texture) {
            ...
            UIAtlas.prototype.loadUseImg = function ($fun) {
                ...
                this.useImg.src = me.Scene_data.fileRoot + this._useImgUrl;
            };
            UIAtlas.prototype.getRec = function ($name) {
                var rec = new me.UIRectangle;
            ...
            UIAtlas.prototype.getGridRec = function ($name) {
                var rec = new me.UIGridRentangle;
                for (var j = 0; j < this.configData.length; j++) {
                    if (this.configData[j].name == $name) {
                        rec.x = this.configData[j].x;
                        rec.y = this.configData[j].y;
            ...
            UIAtlas.prototype.updateCtx = function ($ctx, xpos, ypos) {
                me.TextureManager.getInstance().updateTexture(this.texture, xpos, ypos, $ctx);
            };
            UIAtlas.prototype.upDataPicToTexture = function ($url, $iconName) {
                var _this = this;
                me.LoadManager.getInstance().load(me.Scene_data.fileRoot + $url, me.LoadManager.IMG_TYPE, function ($img) {
                    var rec = _this.getRec($iconName);
                    _this.ctx = me.UIManager.getInstance().getContext2D(rec.pixelWitdh, rec.pixelHeight, false);
                    _this.ctx.drawImage($img, 0, 0, rec.pixelWitdh, rec.pixelHeight);
                    me.TextureManager.getInstance().updateTexture(_this.texture, rec.pixelX, rec.pixelY, _this.ctx);
                });
            };
            UIAtlas.prototype.upDataWebPicToTexture = function ($url, $iconName) {
                var _this = this;
                me.LoadManager.getInstance().load($url, me.LoadManager.IMG_TYPE, function ($img) {
                    var rec = _this.getRec($iconName);
                    _this.ctx = me.UIManager.getInstance().getContext2D(rec.pixelWitdh, rec.pixelHeight, false);
                    _this.ctx.drawImage($img, 0, 0, rec.pixelWitdh, rec.pixelHeight);
                    me.TextureManager.getInstance().updateTexture(_this.texture, rec.pixelX, rec.pixelY, _this.ctx);
            ...
            UIAtlas.prototype.clearCtxTextureBySkilname = function ($iconName) {
                ...
                this.ctx = me.UIManager.getInstance().getContext2D(rec.pixelWitdh, rec.pixelHeight, false);
                this.ctx.clearRect(0, 0, rec.pixelWitdh, rec.pixelHeight);
                me.TextureManager.getInstance().updateTexture(this.texture, rec.pixelX, rec.pixelY, this.ctx);
            };
            UIAtlas.prototype.copyPicToTexture = function ($srcSkin, $desSkin) {
                var srcRec = this.getRec($srcSkin);
                var desRec = this.getRec($desSkin);
                this.ctx = me.UIManager.getInstance().getContext2D(desRec.pixelWitdh, desRec.pixelHeight, false);
                ...
                me.TextureManager.getInstance().updateTexture(this.texture, desRec.pixelX, desRec.pixelY, this.ctx);
            };
            /**
             * 渲染文字
             */
            UIAtlas.prototype.updateLable = function ($key, $str, fontsize, fontColor, textBaseline, textAlign, bolder, maxWidth) {
                if (textBaseline === void 0) { textBaseline = me.TextAlign.MIDDLE; }
                if (textAlign === void 0) { textAlign = me.TextAlign.CENTER; }
            ...
            UIAtlas.prototype.updateArtNum = function ($targetName, $srcName, num) {
                ...
                this.ctx = me.UIManager.getInstance().getContext2D(targetRec.pixelWitdh, targetRec.pixelHeight, false);
                ...
                me.TextureManager.getInstance().updateTexture(this.texture, targetRec.pixelX, targetRec.pixelY, this.ctx);
            };
            //写入单行颜色字体，字号,对齐，基础颜色 并上传显卡
            UIAtlas.prototype.writeSingleLabel = function ($key, $str, fontsize, $align, $baseColor) {
                if (fontsize === void 0) { fontsize = 12; }
                if ($align === void 0) { $align = me.TextAlign.CENTER; }
                if ($baseColor === void 0) { $baseColor = "#ffffff"; }
                me.LabelTextFont.writeSingleLabel(this, $key, $str, fontsize, $align, $baseColor);
            };
            //单行字绘制到CXT上
            UIAtlas.prototype.writeSingleLabelToCxt = function ($ctx, $str, fontsize, $tx, $ty) {
                ...
                if ($tx === void 0) { $tx = 0; }
                if ($ty === void 0) { $ty = 0; }
                $ctx.textBaseline = me.TextAlign.TOP;
                $ctx.textAlign = me.TextAlign.LEFT;
                $ctx.font = (true ? "bolder " : "") + " " + fontsize + "px " + me.UIData.font;
                me.TextRegExp.wrapText($ctx, $str, "#ffffff", $tx, $ty, 9999, 20); //9999为无限大
            };
            /**
            ...
            UIAtlas.prototype.updateLableCtx = function ($ctx, $str, $x, $y, $fontsize, $textAlign, $textBaseline, $textcolor, $textbolder, $maxWidth) {
                ...
                $ctx.font = $textbolder + " " + $fontsize + "px " + me.UIData.font;
            ...
            UIAtlas.prototype.getTextCtx = function ($rec, $fontsize, $fontColor, $bolder, $textBaseline, $textAlign) {
                ...
                $ctx.font = ($bolder ? "bolder " : "") + " " + $fontsize + "px " + me.UIData.font;
                return $ctx;
            };
            ...
            UIAtlas.prototype.getTextypos = function ($textBaseline, $ctx) {
                var $ypos = 0;
                if ($textBaseline == me.TextAlign.BOTTOM) {
                    $ypos = $ctx.canvas.height;
                }
                else if ($textBaseline == me.TextAlign.TOP) {
                    $ypos = 0;
                }
                else if ($textBaseline == me.TextAlign.MIDDLE) {
