/**
* ShopModel
*/
/** 商城标签 */
var shopMediatorType;
(function (shopMediatorType) {
    /** 商会 */
    shopMediatorType[shopMediatorType["SHANGHUI"] = 0] = "SHANGHUI";
    /** 商城 */
    shopMediatorType[shopMediatorType["SHANGCHANG"] = 1] = "SHANGCHANG";
    /** 充值 */
    shopMediatorType[shopMediatorType["CHONGZHI"] = 2] = "CHONGZHI";
})(shopMediatorType || (shopMediatorType = {}));
/** 商会一级菜单 */
var firstMenuType;
(function (firstMenuType) {
    /** 宝石 */
    firstMenuType[firstMenuType["BAOSHI"] = 1] = "BAOSHI";
    /** 装备 */
    firstMenuType[firstMenuType["ZHUANGBEI"] = 2] = "ZHUANGBEI";
    /** 宠物 */
    firstMenuType[firstMenuType["PET"] = 3] = "PET";
    /** 其它 */
    firstMenuType[firstMenuType["QITA"] = 4] = "QITA";
})(firstMenuType || (firstMenuType = {}));
/** 查询商品购售类型 */
var queryType;
(function (queryType) {
    /** 1商品限购次数查询 */
    queryType[queryType["SHOP_BUY_TIMES"] = 1] = "SHOP_BUY_TIMES";
    /** 2商品限售次数查询*/
    queryType[queryType["SHOP_SALE_TIMES"] = 2] = "SHOP_SALE_TIMES";
})(queryType || (queryType = {}));
/** shop购买类型 */
var ShopBuyType;
(function (ShopBuyType) {
    /** 普通商店 */
    ShopBuyType[ShopBuyType["NORMAL_SHOP"] = 1] = "NORMAL_SHOP";
    /** 宠物商店 */
    ShopBuyType[ShopBuyType["PET_SHOP"] = 2] = "PET_SHOP";
    /** 商城 */
    ShopBuyType[ShopBuyType["MALL_SHOP"] = 3] = "MALL_SHOP";
    /** 商会买 */
    ShopBuyType[ShopBuyType["CHAMBER_OF_COMMERCE_SHOP_BUY"] = 4] = "CHAMBER_OF_COMMERCE_SHOP_BUY";
    /** 商会卖 */
    ShopBuyType[ShopBuyType["CHAMBER_OF_COMMERCE_SHOP_SALE"] = 5] = "CHAMBER_OF_COMMERCE_SHOP_SALE";
    /** 兑换商店 */
    ShopBuyType[ShopBuyType["EXCHANGE_BUY"] = 6] = "EXCHANGE_BUY";
    /** 神兽商店 */
    ShopBuyType[ShopBuyType["SHENSHOU_SHOP"] = 7] = "SHENSHOU_SHOP";
    /** 天梯商店买 */
    ShopBuyType[ShopBuyType["TIANTI_SHOP_BUY"] = 8] = "TIANTI_SHOP_BUY";
    /** 天梯商店卖 */
    ShopBuyType[ShopBuyType["TIANTI_SHOP_SALE"] = 9] = "TIANTI_SHOP_SALE";
})(ShopBuyType || (ShopBuyType = {}));
var game;
(function (game) {
    var modules;
    (function (modules) {
        var shop;
        (function (shop) {
            var models;
            (function (models) {
                var ShopModel = /** @class */ (function () {
                    function ShopModel() {
                        /** vip奖励 */
                        this.VipInfoBinDic = {};
                        /** 商品表 */
                        this.GoodsBinDic = {};
                        /**商品表、根据类型对应itemId取值 */
                        this.cGoodsDataForItemId = {};
                        /** 妖魂玉商店的商品配置表 */
                        this.cShenShouShop = {};
                        /** 商品表通过type取值 */
                        this.GoodsAtLimitType = new Laya.Dictionary;
                        /** 一级菜单 */
                        this.CommerceFirstMenuBinDic = {};
                        /** 二级菜单 */
                        this.CommerceSecondMenuBinDic = {};
                        /** 充值配置表 */
                        this.CashluaBinDic = {};
                        /** 商品分类表 */
                        this.MallShopBinDic = {};
                        /** 商品分类表通过type取值 */
                        this.MallShopAtType = new Laya.Dictionary();
                        /** 充值列表信息 */
                        this.goods = [];
                        /** 商会物品id，跳转时使用-显示需求标签 */
                        this.itemId = 0;
                        /** 商城物品id, 跳转时使用-显示需求标签 */
                        this.scItemId = 0;
                        /** 把商会数据以map形式存储 key-商品id,val-商品详细信息 */
                        this.shopInfoBinDic = new Laya.Dictionary();
                        /** 商会商城商品id加载，用于查询商品已购数量 */
                        this.shangHuiArr = [];
                        this.shangChengArr = [];
                        this.shopArr = [];
                        ShopModel._instance = this;
                        game.modules.pet.models.PetProxy.getInstance().on(game.modules.pet.models.SHOPPRICE_EVENT, this, this.setShangHuiData);
                        this.goodsSaleLimit = new Laya.Dictionary();
                    }
                    ShopModel.getInstance = function () {
                        if (!this._instance) {
                            this._instance = new ShopModel();
                        }
                        return this._instance;
                    };
                    ShopModel.clearModelData = function () {
                        shop.models.ShopModel._instance.goods = [];
                        shop.models.ShopModel._instance.chargeStateVo = new models.ChargeStateVo();
                        shop.models.ShopModel._instance.fushiNumVo = new models.FushiNumVo();
                        shop.models.ShopModel._instance.listchargereturnprofit = [];
                        shop.models.ShopModel._instance.notifyBuySuccessVo = new models.NotifyBuySuccessVo();
                        shop.models.ShopModel._instance.itemNumVo = new models.ItemNumVo();
                        shop.models.ShopModel._instance.currency = new Laya.Dictionary();
                        shop.models.ShopModel._instance.goodslimitsBinDic = new Laya.Dictionary();
                        shop.models.ShopModel._instance.goodsSaleLimit = new Laya.Dictionary();
                        shop.models.ShopModel._instance.itemId = 0;
                        shop.models.ShopModel._instance.scItemId = 0;
                    };
                    ShopModel.prototype.setShangHuiData = function () {
                        var _shopInfo = game.modules.pet.models.PetModel.getInstance().shopinfo;
                        for (var i = 0; i < _shopInfo.length; i++) {
                            this.shopInfoBinDic.set(_shopInfo[i].goodsid, _shopInfo[i]);
                        }
                    };
                    ShopModel.prototype.setShopArr = function () {
                        var arr1 = this.GoodsAtLimitType.get(1);
                        for (var i in arr1) {
                            this.shangHuiArr.push(arr1[i].id);
                        }
                        for (var bin in this.GoodsBinDic) {
                            if (this.GoodsBinDic[bin].id >= 4000 && this.GoodsBinDic[bin].id < 5000) {
                                if (this.GoodsBinDic[bin].limitType > 0 || this.GoodsBinDic[bin].id == 4040) {
                                    this.shangChengArr.push(this.GoodsBinDic[bin].id);
                                }
                            }
                            if (this.GoodsBinDic[bin].limitType > 0) {
                                this.shopArr.push(this.GoodsBinDic[bin].id);
                            }
                        }
                    };
                    /** 获取物品图标 */
                    ShopModel.prototype.getSrc = function (index) {
                        var src = "";
                        if (index <= 10000) {
                            src = "common/icon/skill/" + index + ".png";
                        }
                        else if (index <= 10500) {
                            src = "common/icon/bustrole/" + index + ".png";
                        }
                        else if (index <= 11000) {
                            src = "common/icon/bustmonster/" + index + ".png";
                        }
                        else if (index <= 11100) {
                            src = "common/icon/bustpartner/" + index + ".png";
                        }
                        else if (index <= 11200) {
                            src = "common/icon/bustmount/" + index + ".png";
                        }
                        else if (index <= 12000) {
                            src = "common/icon/bustpet/" + index + ".png";
                        }
                        else if (index <= 30000) {
                            src = "common/icon/item/" + index + ".png";
                        }
                        else if (index <= 30500) {
                            src = "common/icon/avatarrole/" + index + ".png";
                        }
                        else if (index <= 31000) {
                            src = "common/icon/avatarmonster/" + index + ".png";
                        }
                        else if (index <= 31100) {
                            src = "common/icon/avatarpartner/" + index + ".png";
                        }
                        else if (index <= 31200) {
                            src = "common/icon/avatarmount/" + index + ".png";
                        }
                        else if (index <= 32000) {
                            src = "common/icon/avatarpet/" + index + ".png";
                        }
                        else if (index <= 40500) {
                            src = "common/icon/grayavatarrole/" + index + ".png";
                        }
                        else if (index <= 41000) {
                            src = "common/icon/grayavatarmonster/" + index + ".png";
                        }
                        else if (index <= 41100) {
                            src = "common/icon/grayavatarpartner/" + index + ".png";
                        }
                        else if (index <= 41200) {
                            src = "common/icon/grayavatarmount/" + index + ".png";
                        }
                        else if (index <= 42000) {
                            src = "common/icon/grayavatarpet/" + index + ".png";
                        }
                        return src;
                    };
                    return ShopModel;
                }());
                models.ShopModel = ShopModel;
            })(models = shop.models || (shop.models = {}));
        })(shop = modules.shop || (modules.shop = {}));
    })(modules = game.modules || (game.modules = {}));
})(game || (game = {}));
//# sourceMappingURL=ShopModel.js.map