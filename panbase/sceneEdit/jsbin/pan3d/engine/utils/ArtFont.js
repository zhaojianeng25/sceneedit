...
(function (Pan3d) {
    ...
    (function (me) {
        var ArtFont = /** @class */ (function () {
            function ArtFont() {
                this.fontData = new me.Dictionary([]);
                this.fontData_Unit = new me.Dictionary([]);
                this.makeFontRect();
            }
            ...
            ArtFont.prototype.makeFontRect = function () {
                this.fontData["Red"] = this.makeBase12pxNum(new me.Vector2D(0, 0));
                this.fontData["Green"] = this.makeBase12pxNum(new me.Vector2D(0, 15));
                this.fontData["Blue"] = this.makeBase12pxNum(new me.Vector2D(0, 30));
                this.fontData["White"] = this.makeBase12pxNum(new me.Vector2D(0, 45));
                this.fontData["Yellow"] = this.makeBase12pxNum(new me.Vector2D(0, 75));
                this.getXmlData();
            };
            ArtFont.prototype.getXmlData = function () {
                // var obj: any = UIData.getUiByName(UIData.textlist, "A_txt1");
                var $uiList = me.UIData.getUiArrByKey(me.UIData.textlist);
                for (var i = 0; i < $uiList.length; i++) {
                    var $skinName = String($uiList[i].name);
                    if ($skinName.search("A_") != -1) {
                        var $id = Number($skinName.substring(5, $skinName.length));
                        var $uiRect = new me.UIRectangle();
                        ...
                        var $rectFont = new me.Rectangle(0, 0, $uiRect.pixelWitdh / $uiRect.cellX, $uiRect.pixelHeight);
                        var $arr = new Array;
                        for (var j = 0; j < $uiRect.cellX; j++) {
                            $arr.push(new me.Rectangle($uiRect.pixelX + j * $rectFont.width, $uiRect.pixelY, $rectFont.width, $rectFont.height));
                        }
            ...
            ArtFont.prototype.makeBase12pxNum = function ($pos) {
                var $arr = new Array;
                $arr.push(new me.Rectangle(3, 0, 9, 14)); //0
                $arr.push(new me.Rectangle(14, 0, 9, 14)); //1
                $arr.push(new me.Rectangle(25, 0, 9, 14)); //2
                $arr.push(new me.Rectangle(36, 0, 9, 14)); //3
                $arr.push(new me.Rectangle(47, 0, 9, 14)); //4
                $arr.push(new me.Rectangle(59, 0, 9, 14)); //5
                $arr.push(new me.Rectangle(70, 0, 9, 14)); //6
                $arr.push(new me.Rectangle(81, 0, 9, 14)); //7
                $arr.push(new me.Rectangle(92.5, 0, 9, 14)); //8
                $arr.push(new me.Rectangle(104, 0, 9, 14)); //9
                $arr.push(new me.Rectangle(116, 0, 9, 14)); //<+>
                $arr.push(new me.Rectangle(126, 0, 9, 14)); //<->
                $arr.push(new me.Rectangle(136, 0, 9, 14)); //</>
                $arr.push(new me.Rectangle(144, 0, 9, 13)); //<:>
                $arr.push(new me.Rectangle(154, 0, 9, 13)); //<.>
            ...
            ArtFont.prototype.writeFontToCtxLeft = function ($ctx, $str, $color, $tx, $ty, $txtInterval) {
                ...
                for (var i = 0; i < $str.length; i++) {
                    ...
                    $ctx.drawImage(me.UIData.textImg, txtNumRect.x, txtNumRect.y, txtNumRect.width, txtNumRect.height, totalW + $tx, $ty, txtNumRect.width, txtNumRect.height);
            ...
            ArtFont.prototype.writeFontToCtxRight = function ($ctx, $str, $color, $tx, $ty, $txtInterval) {
                ...
                for (var i = 0; i < $str.length; i++) {
                    numId = this.getCharId($str[i]);
                    var txtNumRect = this.getRect(numId, $color, $textItem);
                    $ctx.drawImage(me.UIData.textImg, txtNumRect.x, txtNumRect.y, txtNumRect.width, txtNumRect.height, totalW + $tx - $txtWidth, $ty, txtNumRect.width, txtNumRect.height);
                    totalW = totalW + (txtNumRect.width - $txtInterval);
                }
                return $txtWidth;
            };
            ...
            ArtFont.prototype.writeFontToSkinName = function ($UIAtlas, $iconName, $str, $color, $textAlign, $txtInterval) {
                ...
                if ($textAlign === void 0) { $textAlign = me.TextAlign.LEFT; }
                if ($txtInterval === void 0) { $txtInterval = 0; }
                var rec = $UIAtlas.getRec($iconName);
                $UIAtlas.ctx = me.UIManager.getInstance().getContext2D(rec.pixelWitdh, rec.pixelHeight, false);
                var $txtnum;
                if ($textAlign == me.TextAlign.LEFT) {
                    $txtnum = this.writeFontToCtxLeft($UIAtlas.ctx, $str, $color, 0, 0, $txtInterval);
                }
                else if ($textAlign == me.TextAlign.RIGHT) {
                    $txtnum = this.writeFontToCtxRight($UIAtlas.ctx, $str, $color, rec.pixelWitdh, 0, $txtInterval);
                }
                else if ($textAlign == me.TextAlign.CENTER) {
                    $txtnum = this.writeFontToCtxCenten($UIAtlas.ctx, $str, $color, rec.pixelWitdh / 2, 0, $txtInterval);
                }
                me.TextureManager.getInstance().updateTexture($UIAtlas.texture, rec.pixelX, rec.pixelY, $UIAtlas.ctx);
                return $txtnum;
            };
            ArtFont.prototype.writeFontToSkinNameCenter = function ($UIAtlas, $iconName, $str, $color, $txtInterval) {
                ...
                for (var i = 0; i < $str.length; i++) {
                    numId = this.getCharId($str[i]);
                    var txtNumRect = this.getRect(numId, $color, $textItem);
                    $UIAtlas.ctx.drawImage(me.UIData.textImg, txtNumRect.x, txtNumRect.y, txtNumRect.width, txtNumRect.height, totalW + $tx - $txtWidth, $ty - (txtNumRect.height / 2), txtNumRect.width, txtNumRect.height);
                    totalW = totalW + (txtNumRect.width - $txtInterval);
                }
                me.TextureManager.getInstance().updateTexture($UIAtlas.texture, rec.pixelX, rec.pixelY, $UIAtlas.ctx);
            };
            ...
            ArtFont.prototype.upArtFont = function ($UIAtlas, $iconName, $str, $size, $color, $textAlign) {
                ...
                //选择左中右排布
                var xpos = this.getXpos($textAlign, totalW, new me.Rectangle(0, 0, rec.pixelWitdh, rec.pixelHeight));
                //绘制
            ...
            ArtFont.prototype.getTotalWandDraw = function ($rect, $str, $textItem, $scale, $ctx, $isCtx, $xpos, $txtInterval) {
                ...
                for (var i = 0; i < $str.length; i++) {
                    ...
                    if ($isCtx) {
                        $ctx.drawImage(me.UIData.textImg, txtNumRect.x, txtNumRect.y, txtNumRect.width, txtNumRect.height, $rect.x + totalW + $xpos, $rect.y, txtNumRect.width * $scale, txtNumRect.height * $scale);
                    }
