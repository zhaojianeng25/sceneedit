    ...
    (function (me) {
        var MeshDataManager = /** @class */ (function (_super) {
            ...
            MeshDataManager.prototype.getMeshData = function ($url, $fun, $batchNum) {
                ...
                this._loadDic[$url].push($fun);
                me.ResManager.getInstance().loadRoleRes(me.Scene_data.fileRoot + $url, function ($roleRes) {
            ...
            MeshDataManager.prototype.readData = function (byte, $batchNum, $url, $version) {
                ...
                $skinMesh.hitBox = new me.Vector2D(20, 20);
                if ($version >= 23) {
                ...
                for (var i = 0; i < meshNum; i++) {
                    ...
                    if ($version >= 21) {
                        ...
                        me.BaseRes.readFloatTwoByte(byte, meshData.vertices);
                        me.BaseRes.readFloatTwoByte(byte, meshData.tangents);
                        me.BaseRes.readFloatTwoByte(byte, meshData.bitangents);
                        me.BaseRes.readFloatTwoByte(byte, meshData.normals);
                        me.BaseRes.readFloatTwoByte(byte, meshData.uvs);
                        me.BaseRes.readIntForOneByte(byte, meshData.boneIDAry);
                        me.BaseRes.readFloatOneByte(byte, meshData.boneWeightAry);
                        me.BaseRes.readIntForTwoByte(byte, meshData.indexs);
                        me.BaseRes.readIntForTwoByte(byte, meshData.boneNewIDAry);
                    ...
                    meshData.materialParamData = me.BaseRes.readMaterialParamData(byte);
                    var particleNum = byte.readInt();
                    for (var j = 0; j < particleNum; j++) {
                        var bindParticle = new me.BindParticle(byte.readUTF(), byte.readUTF());
                ...
                for (var j = 0; j < sokcetLenght; j++) {
                    var boneData = new me.BoneSocketData();
            ...
            MeshDataManager.prototype.readMesh2OneBuffer = function (byte, meshData) {
                ...
                me.BaseRes.readBytes2ArrayBuffer(byte, data, 3, 0, dataWidth); //vertices
                me.BaseRes.readBytes2ArrayBuffer(byte, data, 2, uvsOffsets, dataWidth); //uvs
                me.BaseRes.readBytes2ArrayBuffer(byte, data, 3, normalsOffsets, dataWidth); //normals
                me.BaseRes.readBytes2ArrayBuffer(byte, data, 3, tangentsOffsets, dataWidth); //tangents
                me.BaseRes.readBytes2ArrayBuffer(byte, data, 3, bitangentsOffsets, dataWidth); //bitangents
                me.BaseRes.readBytes2ArrayBuffer(byte, data, 4, boneIDOffsets, dataWidth, 2); //boneIDAry
                me.BaseRes.readBytes2ArrayBuffer(byte, data, 4, boneWeightOffsets, dataWidth, 1); //boneWeightAry
                ...
                meshData.vertexBuffer = me.Scene_data.context3D.uploadBuff3DArrayBuffer(arybuff);
                meshData.indexBuffer = me.Scene_data.context3D.uploadIndexBuff3D(meshData.indexs);
            };
            MeshDataManager.prototype.uploadMesh = function ($mesh) {
                $mesh.vertexBuffer = me.Scene_data.context3D.uploadBuff3D($mesh.vertices);
                $mesh.uvBuffer = me.Scene_data.context3D.uploadBuff3D($mesh.uvs);
                $mesh.boneIdBuffer = me.Scene_data.context3D.uploadBuff3D($mesh.boneIDAry);
                $mesh.boneWeightBuffer = me.Scene_data.context3D.uploadBuff3D($mesh.boneWeightAry);
                $mesh.indexBuffer = me.Scene_data.context3D.uploadIndexBuff3D($mesh.indexs);
            };
            MeshDataManager.prototype.uploadPbrMesh = function ($mesh, $useNormal) {
                $mesh.normalsBuffer = me.Scene_data.context3D.uploadBuff3D($mesh.normals);
                if ($useNormal) {
                    $mesh.tangentBuffer = me.Scene_data.context3D.uploadBuff3D($mesh.tangents);
                    $mesh.bitangentBuffer = me.Scene_data.context3D.uploadBuff3D($mesh.bitangents);
                }
            };
        ...
        me.MeshDataManager = MeshDataManager;
