    ...
    (function (me) {
        var Display3dMovie = /** @class */ (function (_super) {
            ...
            Display3dMovie.prototype.setRoleUrl = function (value) {
                var _this = this;
                this.clearMesh();
                me.MeshDataManager.getInstance().getMeshData(value, function ($skinMesh) {
            ...
            Display3dMovie.prototype.onMeshLoaded = function () {
                this.dispatchEvent(new me.BaseEvent(me.BaseEvent.COMPLETE));
            };
            ...
            Display3dMovie.prototype.addSkinMeshParticle = function () {
                ...
                for (var i = 0; i < meshAry.length; i++) {
                    ...
                    for (var j = 0; j < particleAry.length; j++) {
                        ...
                        var particle;
                        particle = me.ParticleManager.getInstance().getParticleByte(me.Scene_data.fileRoot + bindPartcle.url);
            ...
            Display3dMovie.prototype.removeSkinMeshParticle = function () {
                ...
                for (var i = 0; i < dicAry.length; i++) {
                    me.ParticleManager.getInstance().removeParticle(dicAry[i]);
                    dicAry[i].destory();
                }
            ...
            Display3dMovie.prototype.setMeshUrl = function (value, $batchNum) {
                ...
                this._meshUrl = me.Scene_data.fileRoot + value;
                me.MeshDataManager.getInstance().getMeshData(this._meshUrl, function ($skinMesh) {
                    _this._skinMesh = $skinMesh;
                    if ($batchNum != 1) {
            ...
            Object.defineProperty(Display3dMovie.prototype, "shadow", {
                set: function (value) {
                    if (value) {
                        if (!this._shadow) {
                            this._shadow = me.ShadowManager.getInstance().addShadow();
                        }
                    }
                    else {
                        if (this._shadow) {
                            me.ShadowManager.getInstance().removeShadow(this._shadow);
                        }
                    }
                },
                enumerable: true,
            ...
            Display3dMovie.prototype.removeStage = function () {
                ...
                for (var key in this._partDic) {
                    ...
                    for (var i = 0; i < ary.length; i++) {
                        if (ary[i] instanceof me.CombineParticle) {
                            me.ParticleManager.getInstance().removeParticle(ary[i]);
                        }
                        else if (ary[i] instanceof me.Display3DSprite) {
                            me.SceneManager.getInstance().removeSpriteDisplay(ary[i]);
                        }
                    }
                }
            };
            ...
            */
            Display3dMovie.prototype.addPart = function ($key, $bindSocket, $url) {
                ...
                var ary = this._partDic[$key];
                me.GroupDataManager.getInstance().getGroupData(me.Scene_data.fileRoot + $url, function (groupRes) {
            ...
            Display3dMovie.prototype.loadPartRes = function ($bindSocket, groupRes, ary) {
                ...
                for (var i = 0; i < groupRes.dataAry.length; i++) {
                    ...
                    var scaleV3d;
                    if (item.isGroup) {
                        posV3d = new me.Vector3D(item.x, item.y, item.z);
                        rotationV3d = new me.Vector3D(item.rotationX, item.rotationY, item.rotationZ);
                        scaleV3d = new me.Vector3D(item.scaleX, item.scaleY, item.scaleZ);
                    }
                    if (item.types == me.BaseRes.SCENE_PARTICLE_TYPE) {
                        var particle = me.ParticleManager.getInstance().getParticleByte(me.Scene_data.fileRoot + item.particleUrl);
                        ary.push(particle);
                        particle.bindTarget = this;
                        ...
                        var display = new me.Display3DSprite();
                        ...
                        me.SceneManager.getInstance().addSpriteDisplay(display);
                        if (item.isGroup) {
            ...
            Display3dMovie.prototype.getSocket = function (socketName, resultMatrix) {
                ...
                resultMatrix.appendRotation(boneSocketData.rotationX, me.Vector3D.X_AXIS);
                resultMatrix.appendRotation(boneSocketData.rotationY, me.Vector3D.Y_AXIS);
                resultMatrix.appendRotation(boneSocketData.rotationZ, me.Vector3D.Z_AXIS);
            ...
            Display3dMovie.prototype.setAnimUrl = function (name, url) {
                ...
                me.AnimManager.getInstance().getAnimData(url, function ($animData) {
            ...
            Display3dMovie.prototype.update = function () {
                ...
                if (this.lightProbe) {
                    this.resultSHVec = me.LightProbeManager.getInstance().getData(new me.Vector3D(this.x, this.y + 10, this.z));
                }
                // if(this.name == "老鹰"){
            ...
            Display3dMovie.prototype.updateFrame = function (t) {
                ...
                this._curentFrame = float2int(this._actionTime / (me.Scene_data.frameTime * 2));
                if (this._curentFrame >= animData.matrixAry.length) {
            ...
            Display3dMovie.prototype.updateShowCapsule = function () {
                if (this.capsuleLineSprite) {
                    ...
                    this.capsuleLineSprite = new me.LineDisplaySprite();
                    this.capsuleLineSprite.clear();
                    this.capsuleLineSprite.baseColor = new me.Vector3D(1, 0, 0, 1);
            ...
            Display3dMovie.prototype.drawBall = function ($r) {
                ...
                for (j = 0; j <= num; j++) {
                    lastPos = null;
                    for (i = num / 2; i < num; i++) {
                        p = new me.Vector3D(radiusNum100, 0, 0);
                        m = new me.Matrix3D;
                        m.appendRotation((360 / num) * i, me.Vector3D.Z_AXIS);
                        p = m.transformVector(p);
                        bm = new me.Matrix3D;
                        bm.appendRotation((360 / num) * j, me.Vector3D.Y_AXIS);
                ...
                for (j = 1; j <= 4; j++) {
                    bm = new me.Matrix3D;
                    bm.appendRotation(j * -20, me.Vector3D.Z_AXIS);
                    bp = bm.transformVector(new me.Vector3D(radiusNum100, 0, 0));
                    lastPos = null;
                    for (i = 0; i < num; i++) {
                        p = bp.clone();
                        m = new me.Matrix3D;
                        m.appendRotation((360 / num) * i, me.Vector3D.Y_AXIS);
                        p = m.transformVector(p);
                        if (lastPos) {
                            this.capsuleLineSprite.makeLineMode(lastPos, p);
                        }
            ...
            Display3dMovie.prototype.drawCylinder = function ($w, $h) {
                ...
                var i;
                for (i = 0; i < jindu; i++) {
                    var a = new me.Vector3D(w, 0, 0);
                    var b = new me.Vector3D(w, +h, 0);
                    var m = new me.Matrix3D;
                    m.appendRotation(i * (360 / jindu), me.Vector3D.Y_AXIS);
                    ...
                    this.capsuleLineSprite.makeLineMode(B, new me.Vector3D(0, +h, 0));
                    if (i == (jindu - 1)) {
            ...
            Display3dMovie.prototype.setVcMatrix = function ($mesh) {
                ...
                me.Scene_data.context3D.setVpMatrix($mesh.material.shader, me.Scene_data.vpMatrix.m);
                me.Scene_data.context3D.setVcMatrix4fv($mesh.material.shader, "posMatrix3D", this.posMatrix.m);
            ...
            Display3dMovie.prototype.setVaIndependent = function ($mesh) {
                me.Scene_data.context3D.setVa(0, 3, $mesh.vertexBuffer);
                me.Scene_data.context3D.setVa(1, 2, $mesh.uvBuffer);
                me.Scene_data.context3D.setVa(2, 4, $mesh.boneIdBuffer);
                me.Scene_data.context3D.setVa(3, 4, $mesh.boneWeightBuffer);
                if ($mesh.material.usePbr) {
                    me.Scene_data.context3D.setVa(4, 4, $mesh.normalsBuffer);
                    me.Scene_data.context3D.setVcMatrix4fv($mesh.material.shader, "rotationMatrix3D", this._rotationMatrix.m);
                    if ($mesh.material.useNormal) {
                        me.Scene_data.context3D.setVa(5, 4, $mesh.tangentBuffer);
                        me.Scene_data.context3D.setVa(6, 4, $mesh.bitangentBuffer);
                    }
                }
                else {
            ...
            Display3dMovie.prototype.setVaCompress = function ($mesh) {
                var tf = me.Scene_data.context3D.pushVa($mesh.vertexBuffer);
                if (tf) {
                    ////console.log('cccccc')
                    return;
                }
                me.Scene_data.context3D.setVaOffset(0, 3, $mesh.stride, 0);
                me.Scene_data.context3D.setVaOffset(1, 2, $mesh.stride, $mesh.uvsOffsets);
                me.Scene_data.context3D.setVaOffset(2, 4, $mesh.stride, $mesh.boneIDOffsets);
                me.Scene_data.context3D.setVaOffset(3, 4, $mesh.stride, $mesh.boneWeightOffsets);
                if ($mesh.material.usePbr) {
                    me.Scene_data.context3D.setVaOffset(4, 3, $mesh.stride, $mesh.normalsOffsets);
                    me.Scene_data.context3D.setVcMatrix4fv($mesh.material.shader, "rotationMatrix3D", this._rotationMatrix.m);
                    if ($mesh.material.useNormal) {
                        me.Scene_data.context3D.setVaOffset(5, 3, $mesh.stride, $mesh.tangentsOffsets);
                        me.Scene_data.context3D.setVaOffset(6, 3, $mesh.stride, $mesh.bitangentsOffsets);
                    }
                }
                else {
                    if ($mesh.material.lightProbe || $mesh.material.directLight) {
            ...
            Display3dMovie.prototype.clearVa = function () {
                me.Scene_data.context3D.clearVa(2);
                me.Scene_data.context3D.clearVa(3);
                me.Scene_data.context3D.clearVa(4);
                me.Scene_data.context3D.clearVa(5);
                me.Scene_data.context3D.clearVa(6);
            };
            Display3dMovie.prototype.updateMaterialMesh = function ($mesh) {
                if (!$mesh.material) {
                    return;
                }
                me.Scene_data.context3D.setProgram($mesh.material.shader.program);
                // Scene_data.context3D.cullFaceBack($mesh.material.backCull);
                me.Scene_data.context3D.cullFaceBack(false);
                me.Scene_data.context3D.setBlendParticleFactors($mesh.material.blendMode);
                ...
                this.setMeshVc($mesh);
                me.Scene_data.context3D.drawCall($mesh.indexBuffer, $mesh.treNum);
            };
            Display3dMovie.prototype.setLightProbeVc = function ($material) {
                if ($material.lightProbe) {
                    for (var i = 0; i < this.resultSHVec.length; i++) {
                        me.Scene_data.context3D.setVc3fv($material.shader, "sh[" + i + "]", [this.resultSHVec[i].x, this.resultSHVec[i].y, this.resultSHVec[i].z]);
                    }
                }
            };
            Display3dMovie.prototype.setMeshVc = function ($mesh) {
                ...
                me.Scene_data.context3D.setVc4fv($mesh.material.shader, "boneQ", $dualQuatFrame.quat); //旋转
                me.Scene_data.context3D.setVc3fv($mesh.material.shader, "boneD", $dualQuatFrame.pos); //所有的位移
            };
        ...
        me.Display3dMovie = Display3dMovie;
