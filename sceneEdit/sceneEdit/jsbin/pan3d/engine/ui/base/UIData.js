    ...
    (function (me) {
        var MainUiLoad = /** @class */ (function () {
            ...
            MainUiLoad.loadUIdata = function ($xmlUrl, $imgUrl, $key, $isTexture) {
                ...
                me.LoadManager.getInstance().load(me.Scene_data.fileRoot + $xmlUrl, me.LoadManager.XML_TYPE, function ($data) {
                    var $arr = Array(JSON.parse($data))[0];
                    _this._dic[$key] = $arr;
                    if ($isTexture) {
                        me.TextureManager.getInstance().getTexture(me.Scene_data.fileRoot + $imgUrl, function ($texture) {
                            _this._imgDic[$key] = $texture;
                            _this.loadOkd();
                        }, 1, null, 0);
                    }
                    else {
                        kim.src = me.Scene_data.fileRoot + $imgUrl;
                    }
                });
            };
            ...
            MainUiLoad._dic = new me.Dictionary([]);
            MainUiLoad._imgDic = new me.Dictionary([]);
            return MainUiLoad;
        }());
        me.MainUiLoad = MainUiLoad;
        var UIData = /** @class */ (function () {
            ...
            UIData.resize = function () {
                this.Scale = Math.min(me.Scene_data.stageWidth / this.designWidth, me.Scene_data.stageHeight / this.designHeight);
            };
            ...
            UIData._dic = new me.Dictionary([]);
            UIData._imgDic = new me.Dictionary([]);
            return UIData;
        }());
        me.UIData = UIData;
        var UiDraw = /** @class */ (function () {
            ...
            UiDraw.clearUI = function ($ui) {
                ...
                var $ctx = me.UIManager.getInstance().getContext2D($uiRect.pixelWitdh, $uiRect.pixelHeight, false);
            ...
            UiDraw.drawTxtLab = function ($ui, $str, $fontsize, $align, $tx, $ty) {
                ...
                var $rec = $ui.uiRender.uiAtlas.getRec($ui.skinName);
                var ctx = me.UIManager.getInstance().getContext2D($rec.pixelWitdh, $rec.pixelHeight, false);
                UiDraw.cxtDrawImg(ctx, me.PuiData.TXTBG, new me.Rectangle(0, 0, $rec.pixelWitdh, $rec.pixelHeight), UIData.publicUi);
                me.LabelTextFont.writeSingleLabelToCtx(ctx, $str, $fontsize, $tx, $ty, $align);
                $ui.uiRender.uiAtlas.updateCtx(ctx, $rec.pixelX, $rec.pixelY);
            };
            /**属性 - value */
            UiDraw.drawAttVal = function ($ui, $att, $val, $align, $needadd) {
                if ($align === void 0) { $align = me.TextAlign.LEFT; }
                ...
                keyStr = me.ColorType.Orange7a2f21 + getKeyProById($att) + ":  " + me.ColorType.Orange9a683f + Snum(Math.floor($val / 100));
                if ($needadd) {
                    keyStr = me.ColorType.Orange7a2f21 + getKeyProById($att) + ":  " + me.ColorType.Orange9a683f + "+" + Snum(Math.floor($val / 100));
                }
                me.LabelTextFont.writeSingleLabel($ui.uiRender.uiAtlas, $ui.skinName, keyStr, 14, $align);
            };
            /**绘制未获得属性 - value */
            UiDraw.drawAttValAdd = function ($ui, $att, $val) {
                ...
                keyStr = me.ColorType.Orange7a2f21 + getKeyProById($att) + ":    " + me.ColorType.Green2ca937 + "+" + Snum(Math.floor($val / 100));
                me.LabelTextFont.writeSingleLabel($ui.uiRender.uiAtlas, $ui.skinName, keyStr, 14, me.TextAlign.LEFT);
            };
            /**绘制增加属性 向上箭头 */
            UiDraw.drawAddValTop = function ($ui, $val) {
                ...
                var ctx = me.UIManager.getInstance().getContext2D($rec.pixelWitdh, $rec.pixelHeight, false);
                if (addStr != "") {
                    UiDraw.cxtDrawImg(ctx, me.PuiData.ARROW_TOP, new me.Rectangle(0, 3, 12, 16), UIData.publicUi);
                }
                me.LabelTextFont.writeSingleLabelToCtx(ctx, addStr, 14, 15, 0, me.TextAlign.LEFT, me.ColorType.Green2ca937);
                $ui.uiRender.uiAtlas.updateCtx(ctx, $rec.pixelX, $rec.pixelY);
            };
            /**绘制增加属性 向右箭头 $align只接受左右对齐*/
            UiDraw.drawAddValRight = function ($ui, $val, $needadd, $align) {
                if ($needadd === void 0) { $needadd = false; }
                if ($align === void 0) { $align = me.TextAlign.RIGHT; }
                // var addStr: string = "";
                if ($val >= 0) {
                    // addStr = Snum($val);
                    var $rec = $ui.uiRender.uiAtlas.getRec($ui.skinName);
                    var ctx = me.UIManager.getInstance().getContext2D($rec.pixelWitdh, $rec.pixelHeight, false);
                    UiDraw.cxtDrawImg(ctx, me.PuiData.ARROW_RIGHT, new me.Rectangle(0, 3, 16, 12), UIData.publicUi);
                    var addStr = Snum(Math.floor($val / 100));
                    if ($needadd) {
                        addStr = "+" + addStr;
                    }
                    var tx = 0;
                    if ($align == me.TextAlign.LEFT) {
                        tx = 22;
                    }
                    //console.log("-----addStr----",addStr,tx);
                    me.LabelTextFont.writeSingleLabelToCtx(ctx, addStr, 14, tx, 0, $align, me.ColorType.Green2ca937);
                    $ui.uiRender.uiAtlas.updateCtx(ctx, $rec.pixelX, $rec.pixelY);
                }
                else {
                    UiDraw.clearUI($ui);
            ...
            UiDraw.drawCost = function ($cxt, $tx, $ty, $type) {
                UiDraw.cxtDrawImg($cxt, UIuitl.getInstance().costtype($type), new me.Rectangle($tx, $ty, 35, 35), UIData.publicUi);
            };
            UiDraw.drawCostUI = function ($ui, $tx, $ty, $type) {
                var $goldtxtrec = $ui.uiRender.uiAtlas.getRec($ui.skinName);
                var $ctx = me.UIManager.getInstance().getContext2D($goldtxtrec.pixelWitdh, $goldtxtrec.pixelHeight, false);
                UiDraw.cxtDrawImg($ctx, UIuitl.getInstance().costtype($type), new me.Rectangle($tx, $ty, 35, 35), UIData.publicUi);
            ...
            UiDraw.uiAtlasDrawImg = function ($uiAtlas, $skinName, $key, $shareName) {
                ...
                $uiAtlas.ctx = me.UIManager.getInstance().getContext2D($uiRectangle.pixelWitdh, $uiRectangle.pixelHeight, false);
                ...
                if (obj) {
                    ...
                    me.TextureManager.getInstance().updateTexture($uiAtlas.texture, $uiRectangle.pixelX, $uiRectangle.pixelY, $uiAtlas.ctx);
                }
                else {
            ...
            UiDraw.SharedDrawImg = function ($touiAtlas, $fromuiAtlas, $skinName, $shareName, $tx, $ty, $fillflag) {
                ...
                $touiAtlas.ctx = me.UIManager.getInstance().getContext2D($uiRectangle.pixelWitdh, $uiRectangle.pixelHeight, false);
                ...
                me.TextureManager.getInstance().updateTexture($touiAtlas.texture, $uiRectangle.pixelX, $uiRectangle.pixelY, $touiAtlas.ctx);
            };
            UiDraw.drawToUiAtlasToCtx = function ($ctx, $fromuiAtlas, $shareName, $posRect) {
                var imgUseRect = $fromuiAtlas.getRec($shareName);
                if (!$posRect) {
                    $posRect = new me.Rectangle(0, 0, 0, 0);
                }
            ...
            UiDraw.RepeatLoadImg = function ($url1, $url2, $backFuc) {
                ...
                imgA.onload = function () {
                    me.LoadManager.getInstance().load($url2, me.LoadManager.IMG_TYPE, function ($imgB) {
                        if ($backFuc) {
                            $backFuc(imgA, $imgB);
                        }
                    });
        ...
        var UIuitl = /** @class */ (function () {
            ...
            UIuitl.prototype.drawCostUI = function ($uiAtlas, $skinName, $costary, $fontcolor, $bgwidth, $bgheight) {
                ...
                UiDraw.cxtDrawImg(ctx, me.PuiData.TXTBG, new me.Rectangle($rec.pixelWitdh - $bgwidth, $rec.pixelHeight - $bgheight - 3, $bgwidth, $bgheight + 3), UIData.publicUi);
                var posx;
                var posy;
                if (Number($costary[0]) > -1) {
                    UiDraw.cxtDrawImg(ctx, this.costtype(Number($costary[0])), new me.Rectangle(0, 0, 35, 35), UIData.publicUi);
                ...
                me.LabelTextFont.writeSingleLabelToCtxSetAnchor(ctx, String($costary[1]), 16, posx, posy, me.TextAlign.CENTER, $fontcolor);
                me.ArtFont.getInstance();
            ...
            UIuitl.prototype.costtype = function ($costid) {
                switch ($costid) {
                    case 1:
                        //元宝
                        return me.PuiData.A_YUANBAO;
                    case 2:
                        //绑定元宝
                        return me.PuiData.A_BANGYUAN;
                    case 3:
                        //身上的银子
                        return me.PuiData.A_YINBI;
                    case 101:
                        //真气
                        return me.PuiData.A_ZHENQI;
                    case 102:
                        //兽灵
                        return me.PuiData.A_SHOULING;
                    case 103:
                        //宝石精华
                        return me.PuiData.A_JINGHUA;
                    case 6:
                        //帮贡
                        return me.PuiData.A_BANGGONG;
                    case 7:
                        //荣誉
                        return me.PuiData.A_HONOR;
                    case 8:
                        //斗魂
                        return me.PuiData.A_DOUHUN;
                    case 9:
                        //历练值
                        return me.PuiData.A_DOUHUN;
                    case 10:
                        //怪物令牌
                        return me.PuiData.A_CARD_G;
                    case 11:
                        //采集令牌
                        return me.PuiData.A_CARD_C;
                    case 12:
                        //押镖令牌
                        return me.PuiData.A_CARD_B;
                    case 104:
                        //经验
                        return me.PuiData.A_EXP;
                    default:
                        break;
                }
            };
