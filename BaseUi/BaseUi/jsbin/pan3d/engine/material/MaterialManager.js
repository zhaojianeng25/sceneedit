...
(function (Pan3d) {
    ...
    (function (me) {
        var MaterialManager = /** @class */ (function (_super) {
            ...
            MaterialManager.prototype.getMaterialByte = function ($url, $fun, $info, $autoReg, $regName, $shader3DCls) {
                ...
                if (this._resDic[$url]) {
                    ...
                    me.LoadManager.getInstance().load($url, me.LoadManager.BYTE_TYPE, function ($data, _info) { _this.loadMaterialByteCom($data, _info); }, materialLoad);
                }
            };
            MaterialManager.prototype.meshByteMaterialByt = function (byte, _info) {
                var material = new me.Material();
                material.setByteData(byte);
                material.url = _info.url;
                this.loadMaterial(material);
                if (_info.autoReg) {
                    material.shader = me.ProgrmaManager.getInstance().getMaterialProgram(_info.regName, _info.shader3D, material, null, true);
                }
            ...
            MaterialManager.prototype.loadMaterialByteCom = function ($data, _info) {
                var byte = new me.Pan3dByteArray($data);
            ...
            */
            MaterialManager.prototype.loadMaterial = function ($material) {
                ...
                for (var i = 0; i < texVec.length; i++) {
                    ...
                    me.TextureManager.getInstance().getTexture(me.Scene_data.fileRoot + texVec[i].url, function ($textureVo, $texItem) {
            ...
            MaterialManager.prototype.loadDynamicTexUtil = function (material) {
                ...
                for (var i = 0; i < dynamicTexList.length; i++) {
                    if (dynamicTexList[i].isParticleColor) {
                        ...
                        me.TextureManager.getInstance().getTexture(me.Scene_data.fileRoot + dynamicTexList[i].url, function ($textureVo, $texItem) {
                            $texItem.textureRes = $textureVo;
        ...
        me.MaterialManager = MaterialManager;
