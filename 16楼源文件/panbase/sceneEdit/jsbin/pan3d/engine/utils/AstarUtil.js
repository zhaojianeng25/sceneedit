    ...
    (function (me) {
        var AstarUtil = /** @class */ (function () {
            ...
            AstarUtil.setData = function ($tempNavMesh) {
                ...
                this.aPos = new me.Vector3D(this.navmeshData.aPos.x, this.navmeshData.aPos.y, this.navmeshData.aPos.z);
                ...
                me.SceneManager.getInstance().fixAstart(new me.Vector2D(this.aPos.x, this.midu * this.astarHeight + this.aPos.z));
                this.mathAreaRect();
                this.mathMinMapRect();
            };
            ...
            */
            ...
            AstarUtil.mathAreaRect = function () {
                ...
                */
                this.areaRect = new me.Rectangle;
            ...
            AstarUtil.getHeightByPos = function ($pos) {
                if (this.heightItem) {
                    var $movePos = $pos.subtract(this.aPos).add(new me.Vector3D(this.midu / 2, 0, this.midu / 2));
            ...
            AstarUtil.getWorldPosByStart2D = function (a) {
                if (this.navmeshData) {
                    var Apos = new me.Vector3D(a.x * this.midu, 3, a.y * this.midu);
                    Apos.x = Apos.x + this.aPos.x + this.midu / 2;
                    Apos.z = (this.aPos.z + this.midu * this.astarHeight) - Apos.z - this.midu / 2;
                    return Apos;
                }
                else {
                    return new me.Vector3D(a.x * 10 + this.midu / 2, 0, a.y * 10 - this.midu / 2);
                }
            };
            ...
            AstarUtil.findStraightLine = function ($a, $b) {
                var $nrm = new me.Vector2D($b.x - $a.x, $b.y - $a.y);
                $nrm.normalize();
                var d = Math.round(me.Vector2D.distance($a, $b));
                var p = new me.Vector2D;
                for (var i = 0; i < d; i++) {
            ...
            AstarUtil.findNearLinePoint = function ($a, $b) {
                while (me.Vector3D.distance($a, $b) > 5) {
            ...
            AstarUtil.blockAry = function (ary) {
                ...
                for (var i = 0; i < ary.length; i++) {
                    list.push([new me.Vector2D(ary[i][0], ary[i][1]), new me.Vector2D(ary[i][2], ary[i][3])]);
                }
                this.blockList(list);
            };
            ...
            AstarUtil.getGrapIndexByPos = function ($pos) {
                if (this.navmeshData) {
                    var $movePos = $pos.subtract(this.aPos).add(new me.Vector3D(0, 0, this.midu / 2));
                    var w = this.astarWidth * this.midu;
                    var h = this.astarHeight * this.midu;
                    if ($movePos.x > 0 && $movePos.x < w && $movePos.z > 0 && $movePos.z < h) {
                        return new me.Vector2D(float2int($movePos.x / this.midu), float2int(this.astarHeight - $movePos.z / this.midu));
                    }
                }
                else {
                    return new me.Vector2D(float2int($pos.x / this.midu), float2int($pos.z / this.midu));
                }
                return null;
            };
            AstarUtil.getScenePos = function ($x, $y) {
                var $temp = me.Groundposition.getGroundPos($x, $y);
                return this.getLookAtPos($temp);
            };
            AstarUtil.getLookAtPos = function ($hit3D) {
                var $cam3D = new me.Vector3D(me.Scene_data.cam3D.x, me.Scene_data.cam3D.y, me.Scene_data.cam3D.z);
            ...
            AstarUtil.aPos = new me.Vector3D;
            AstarUtil.midu = 10;
