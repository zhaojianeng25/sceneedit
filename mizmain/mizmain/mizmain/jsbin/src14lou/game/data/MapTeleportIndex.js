...
(function (game) {
    var data;
    (function (data) {
        var MapTeleportIndex = /** @class */ (function () {
            function MapTeleportIndex() {
                /*关闭列表*/
                this._closeList = new Array();
            }
            Object.defineProperty(MapTeleportIndex, "inst", {
                get: function () {
                    if (!this._inst) {
            ...
            Object.defineProperty(MapTeleportIndex.prototype, "data", {
                get: function () {
                    return this._data;
                },
                enumerable: true,
            ...
            //获取某地图所有传送点
            MapTeleportIndex.prototype.getTeleportsByMapid = function (mapid) {
                return this._data[mapid];
            };
            //初始
            MapTeleportIndex.prototype.init = function () {
                //地图个数
                var len = data.Template.data["tb_map"].length;
                this._data = new Array(len);
                var teleport_data = data.Template.data["tb_teleport"];
                if (!teleport_data)
                    return;
                var tele_len = teleport_data.length;
                for (var i = 0; i < tele_len; i++) {
                    var single_tele = teleport_data[i];
                    //地图id
                    var mapid = single_tele.map_id;
                    //冗余，设置数组长度
                    if (mapid >= this._data.length)
                        this._data.length = mapid + 1;
                    //传送点列表
                    if (!this._data[mapid]) {
                        this._data[mapid] = new Array();
                    }
                    var teleport = new data.Teleport();
                    //坐标
                    ...
                    //判断是否单双向
                    if (teleport.dstMapid <= 0 || !single_tele.toward || single_tele.toward < 1)
                        continue;
                    //回去的也要
                    if (!this._data[teleport.dstMapid]) {
                        this._data[teleport.dstMapid] = new Array();
                    }
                    var returnTele = new data.Teleport();
                    //坐标
             ...
             *
             */
            MapTeleportIndex.prototype.find = function (srcMapid, dstMapid) {
             ...
             *
             */
            MapTeleportIndex.prototype.searching = function (firstMapid, exitMapid, ref) {
                //尝试过的点加入列表
                ...
                if (firstMapid == exitMapid)
                    return true;
                //开始遍历所有传送点的目的地查找
                var teleports = this._data[firstMapid];
                if (!teleports)
                    return false;
                var len = teleports.length;
                for (var i = 0; i < len; i++) {
                    var telport = teleports[i];
                    if (!telport)
                        continue;
                    //该地图还未遍历过
            ...
            return MapTeleportIndex;
        }());
        data.MapTeleportIndex = MapTeleportIndex;
    })(data = game.data || (game.data = {}));
})(game || (game = {}));
//# sourceMappingURL=MapTeleportIndex.js.map