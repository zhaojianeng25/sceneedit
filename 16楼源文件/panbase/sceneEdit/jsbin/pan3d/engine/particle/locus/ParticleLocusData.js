    ...
    (function (me) {
        var ParticleLocusData = /** @class */ (function (_super) {
            ...
            ParticleLocusData.prototype.getParticle = function () {
                return new me.Display3DLocusPartilce;
            };
            ParticleLocusData.prototype.setAllByteInfo = function ($byte) {
                ...
                me.BaseRes.readBytes2ArrayBuffer($byte, data, 4, 3, dataWidth, 4); //normal
                me.BaseRes.readBytes2ArrayBuffer($byte, data, 2, 7, dataWidth, 4); //uv
            ...
            ParticleLocusData.prototype.initUV = function () {
                ...
                if (this._isLoop) {
                    if (this._life) {
                        ...
                        $fcVector = new me.Vector3D($moveUv, $lifeRoundNum, -$lifeRoundNum);
                    }
                    else {
                        $moveUv = $moveUv % 1;
                        $fcVector = new me.Vector3D($moveUv + 1, 99, -2);
                    }
                }
                else {
                    if (this._life) {
                        $fcVector = new me.Vector3D($moveUv, $lifeRoundNum, -1);
                    }
                    else {
                        $fcVector = new me.Vector3D($moveUv, 99, -1);
                    }
                }
            ...
            ParticleLocusData.prototype.uploadGpu = function () {
                ...
                if (this._watchEye) {
                    this.objData.normalsBuffer = me.Scene_data.context3D.uploadBuff3D(this.objData.normals);
                }
                this.objData.indexBuffer = me.Scene_data.context3D.uploadIndexBuff3D(this.objData.indexs);
                this.objData.treNum = this.objData.indexs.length;
            };
            ParticleLocusData.prototype.regShader = function () {
                ...
                this.materialParam.shader = me.ProgrmaManager.getInstance().getMaterialProgram(me.Display3DLocusShader.Display3D_Locus_Shader, me.Display3DLocusShader, this.materialParam.material, shaderParameAry);
                this.materialParam.program = this.materialParam.shader.program;
            };
            ParticleLocusData.prototype.initVcData = function () {
                this.vcmatData = new Float32Array(me.Display3DLocusShader.getVcSize() * 16);
            };
            ParticleLocusData.prototype.setFloat32Vec = function (key, ary) {
                var idxary = me.Display3DLocusShader.shader_vec4[key];
                var idx = idxary[0] * 16 + idxary[1] * 4;
                this.vcmatData.set(ary, idx);
            };
            ParticleLocusData.prototype.setFloat32Mat = function (key, ary) {
                var idx = me.Display3DLocusShader.shader_mat4[key] * 16;
                this.vcmatData.set(ary, idx);
            };
            return ParticleLocusData;
        }(me.ParticleData));
        me.ParticleLocusData = ParticleLocusData;
