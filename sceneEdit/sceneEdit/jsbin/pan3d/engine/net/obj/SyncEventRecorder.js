    ...
    (function (me) {
        var SyncEventRecorder = /** @class */ (function (_super) {
            ...
            function SyncEventRecorder() {
                 ...
                 * 用于监听下标变化
                 */
                _this._events_value = new me.NetEventDispatcher();
                /**
                 * 用于监听字符下标变化
                 */
                _this._events_str_values = new me.NetEventDispatcher();
                /**
                 * 用于触发多下标单回调的情况
                 */
                _this._events_mask = new me.NetEventDispatcher(me.NetEventDispatcher.KEY_TYPE_INT_MASK);
                /**
                 * 用于事件回调
                 */
                _this._events_callback = new me.NetEventDispatcher();
                /*整形下标长度*/
                ...
                _this._guid = "";
                //临时变量,每次读取需要使用的临时变量
                _this._tmpBinlog = new me.BinLogStru();
            ...
            SyncEventRecorder.prototype.OnEventSyncBinLog = function (binlog) {
                if (this._after_update) {
                    if (binlog.typ == me.SyncEvent.TYPE_STRING) {
                ...
                else if (binlog.typ == me.SyncEvent.TYPE_STRING) {
            ...
            SyncEventRecorder.prototype.ReadValues = function (mask, bytes, isNew) {
                ...
                for (var i = 0; i < length; i++) {
                    if (mask.GetBit(i)) {
                        this.checkIntSize(i);
                        //从模式需要抛出事件
                        var binlog = !isNew ? me.BinLogStru.malloc() : null;
                        if (binlog) {
                            binlog.typ = me.SyncEvent.TYPE_UINT32;
                            binlog.index = i;
            ...
            SyncEventRecorder.prototype.ReadStringValues = function (mask, bytes, isNew) {
                ...
                for (var i = 0; i < length; i++) {
                    if (mask.GetBit(i)) {
                        ...
                        if (binlog) {
                            binlog.index = i;
                            binlog.typ = me.SyncEvent.TYPE_STRING;
            ...
            SyncEventRecorder.prototype.ApplyAtomicBinLog = function (binlog) {
                ...
                if (binlog.atomic_opt == me.SyncEvent.ATOMIC_OPT_RESULT_FAILED || binlog.atomic_opt == me.SyncEvent.ATOMIC_OPT_RESULT_OK) {
                    this._events_callback.DispatchInt(binlog.callback_idx, binlog);
                    return;
                }
                //字符串分支
                if (binlog._typ == me.SyncEvent.TYPE_STRING) {
                    //如果越界了就扩张
                    ...
                    if (binlog._old_value_str != this._str_values[binlog._index]) {
                        ...
                        binlog._atomic_opt = me.SyncEvent.ATOMIC_OPT_RESULT_FAILED;
                    }
                    else {
                        binlog._atomic_opt = me.SyncEvent.ATOMIC_OPT_RESULT_OK;
                        //应用完后记录一下准备回去了
            ...
            SyncEventRecorder.prototype.ApplyBinLog = function (binlog) {
                ...
                //记录一下旧值
                if (binlog.typ == me.SyncEvent.TYPE_DOUBLE) {
                    ...
                    if (binlog.typ != me.SyncEvent.TYPE_BIT && this._uint32_values_len > index /*&& binlog.opt != OPT_SET*/) {
                ...
                //因为uint32不需要偏移，所以单独写
                if (binlog.typ == me.SyncEvent.TYPE_UINT32 || binlog.typ == me.SyncEvent.TYPE_INT32 || binlog.typ == me.SyncEvent.TYPE_FLOAT) {
                    switch (binlog.opt) {
                        case me.SyncEvent.OPT_SET:
                            //this._uint32_values[index] = binlog.uint32;
                            this.SetUInt32(index, binlog.uint32);
                            break;
                        case me.SyncEvent.OPT_ADD:
                            //this._uint32_values[index] = this._uint32_values[index] + binlog.uint32;
                            this.AddUInt32(index, binlog.uint32);
                            break;
                        case me.SyncEvent.OPT_SUB:
                            //this._uint32_values[index] = this._uint32_values[index] - binlog.uint32;
                            this.SubUInt32(index, binlog.uint32);
                            break;
                    }
                }
                else if (binlog.typ == me.SyncEvent.TYPE_DOUBLE) {
                    switch (binlog.opt) {
                        case me.SyncEvent.OPT_SET:
                ...
                else if (binlog.typ == me.SyncEvent.TYPE_BIT) { //FIXME			
                    switch (binlog.opt) {
                        case me.SyncEvent.OPT_SET:
            ...
            SyncEventRecorder.prototype.ReadFrom = function (flags, bytes, evFilter, applyNew) {
                ...
                var isNew = Boolean(flags & me.SyncEvent.OBJ_OPT_NEW);
                //创建包需要将所有的值清空
                if (isNew) {
                    this.clearValues();
                }
                //创建包或更新包
                if (isNew || flags & me.SyncEvent.OBJ_OPT_UPDATE) {
                    if (evFilter) {
                        evFilter.pushNew();
                    }
                ...
                //如果更新的话可能还带原子操作
                //binlog更新
                if (flags & me.SyncEvent.OBJ_OPT_BINLOG) {
            ...
            SyncEventRecorder.prototype.GetHashCode = function () {
                ...
                var bytes = new me.Pan3dByteArray;
                bytes.endian = me.Endian.LITTLE_ENDIAN;
            ...
            SyncEventRecorder.addListens_mask = new me.UpdateMask;
            SyncEventRecorder.tmpIntMask = new me.UpdateMask;
            SyncEventRecorder.tmpStrMask = new me.UpdateMask;
            return SyncEventRecorder;
        }(me.SyncEvent));
        me.SyncEventRecorder = SyncEventRecorder;
