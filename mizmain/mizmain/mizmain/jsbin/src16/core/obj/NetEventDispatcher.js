...
(function (core) {
    ...
    (function (obj) {
        // 回调结构体
        var HanlderStruct = /** @class */ (function () {
            function HanlderStruct() {
                // 是否释放
                this.dispose = false;
                // 池名称 
                this.poolName = 'HanlderStruct';
            }
            /**
             * 进池 （相当于对象dispose函数）
             */
            HanlderStruct.prototype.intoPool = function () {
                var arg = [];
             ...
             * 出池 （相当于对象初始化函数）
             */
            HanlderStruct.prototype.outPool = function () {
                var arg = [];
                for (var _i = 0; _i < arguments.length; _i++) {
                    arg[_i] = arguments[_i];
                }
            };
            return HanlderStruct;
        }());
        obj.HanlderStruct = HanlderStruct;
        /**
        * 事件分发器,由于本身事件数量肯定不会多
        * 所以没有必要使用二分查找算法,直接遍历
        ...
        */
        var NetEventDispatcher = /** @class */ (function () {
            ...
            NetEventDispatcher.prototype.DispatchIndex = function (param) {
                ...
                if (this._event_key_type == NetEventDispatcher.KEY_TYPE_STRING
                    || this._event_key_type == NetEventDispatcher.KEY_TYPE_INT
                    || this._event_key_type == NetEventDispatcher.KEY_TYPE_INT_MASK) {
                    for (i in this._event_index) {
                        var struct = this._event_callback[this._event_index[i]];
                        struct.method.call(struct.caller, param);
                    }
                }
            };
             ...
             */
            NetEventDispatcher.prototype.AddListenInt = function (key, caller, f) {
                if (this._event_key_type != NetEventDispatcher.KEY_TYPE_INT)
                ...
                for (var i = 0; i < this._event_id_int.length; i++) {
                    if (this._event_id_int[i] == key && this._event_callback[i].caller == caller && this._event_callback[i].method == f) {
                        return;
                    }
                }
                ...
                struct.method = f;
                this._event_callback.push(struct);
            };
            NetEventDispatcher.prototype.AddListenString = function (key, caller, f) {
                if (this._event_key_type != NetEventDispatcher.KEY_TYPE_STRING)
                ...
                for (var i = 0; i < this._event_id_str.length; i++) {
                    if (this._event_id_str[i] == key && this._event_callback[i].caller == caller && this._event_callback[i].method == f) {
                        return;
                    }
                }
            ...
            NetEventDispatcher.prototype.AddListenIntMask = function (key, caller, f) {
                if (this._event_key_type != NetEventDispatcher.KEY_TYPE_INT_MASK)
                ...
                for (var i = 0; i < this._event_id_int_mask.length; i++) {
                    if (this._event_id_int_mask[i].equals(key) && this._event_callback[i].caller == caller && this._event_callback[i].method == f) {
                        return;
                    }
                }
             ...
             */
            NetEventDispatcher.prototype.removeListenerInt = function (key, caller, f) {
                if (f === void 0) { f = null; }
                if (this._event_key_type != NetEventDispatcher.KEY_TYPE_INT)
                ...
                for (var i = 0; i < this._event_id_int.length;) {
                    if (this._event_id_int[i] == key) {
                        var struct = this._event_callback[i];
                        if (f == null || (struct.caller == caller && struct.method == f)) {
             ...
             * @param f 回调函数闭包,可以支持一个参数(Object)，如果f为空，则移除所有
             */
            NetEventDispatcher.prototype.removeListenerString = function (key, caller, f) {
                if (f === void 0) { f = null; }
                if (this._event_key_type != NetEventDispatcher.KEY_TYPE_STRING)
             ...
             * @param f
             */
            NetEventDispatcher.prototype.removeListenerUpdateMask = function (key, caller, f) {
                if (f === void 0) { f = null; }
                if (this._event_key_type != NetEventDispatcher.KEY_TYPE_INT_MASK)
                    throw new Error("removeListenerUpdateMask but ()");
                for (var i = 0; i < this._event_id_int_mask.length;) {
                    if (this._event_id_int_mask[i].equals(key)) {
                        var struct = this._event_callback[i];
                        if (f == null || (struct.caller == caller && struct.method == f)) {
             ...
             */
            NetEventDispatcher.prototype.AddCallback = function (caller, f) {
                if (this._event_key_type == NetEventDispatcher.KEY_TYPE_STRING)
                ...
                this.AddListenInt(new_ev, caller, f);
                return new_ev;
            };
            /**
             * 清空所有已经注册的事件监听
        ...
        obj.NetEventDispatcher = NetEventDispatcher;
