...
var GameApp = /** @class */ (function (_super) {
    ...
    function GameApp() {
        ...
        // 必要素材是否加载完毕
        _this._needAssetLoaded = false;
        // 断线重连是否需要重新获取Session
        _this._reConnectHasGetSession = false;
        _this.init();
        return _this;
    }
    // 初始化函数
    GameApp.prototype.init = function (flag) {
        var _this = this;
        if (!this.preLoad) {
        ...
        // ui初始化后打开登陆页面
        this._uiRoot.on(UIRoot.INIT, this, function () {
            _this.onMPlayerData();
     ...
     * 然后，重新初始化
     */
    GameApp.prototype.removeChilds = function () {
     ...
     * 清除模型
     */
    GameApp.prototype.clearModel = function () {
     ...
     * 清空存放module的字典中的数据
     */
    GameApp.prototype.clearModuleDic = function () {
        ModuleManager.instanceDic = new Laya.Dictionary();
    };
    /**
     * 清除某些定时器
     */
    GameApp.prototype.clearSomeTime = function () {
        Laya.timer.clearAll(this);
    };
    /**
     * 清空所有model里存的数据
     */
    GameApp.prototype.clearAllModleData = function () {
    ...
    // 加载必要素材
    GameApp.prototype.loadNeedAsset = function () {
        var assetsLoader = new AssetsLoader();
        assetsLoader.load([PathConst.template,
            PathConst.atlas_ui + 'hud.atlas',], Handler.create(this, this.onNeedAssetLoaded, [assetsLoader]));
    };
    GameApp.prototype.onNeedAssetLoaded = function (assetsLoader) {
        this._needAssetLoaded = true;
        // 清理载模板预加
        this.preLoad.clear(PathConst.template);
        var buff = Laya.loader.getRes(PathConst.template);
        // 清理模板素材内存
        assetsLoader.release(PathConst.template, true);
        var dataStr = StringU.readZlibData(new ByteArray(buff));
    ...
    // 校验进入游戏
    GameApp.prototype.checkInGame = function () {
        if (!this._needAssetLoaded) {
            //logd('[checkInGame] _needAssetLoaded is false');
            return;
        }
        var loadImg = this._uiRoot.top.getPage(PageDef.LOAD);
        if (loadImg)
            loadImg.showJindu(2);
    };
    // 主玩家数据下来了
    GameApp.prototype.onMPlayerData = function () {
        logd('主玩家数据下来了');
    ...
    // 尝试初始化场景
    GameApp.prototype.tryCreatedSceneRoot = function () {
        var _this = this;
        //初始化场景
        if (!this._sceneRoot) {
            ...
            this._sceneRoot.onTouch = function (cellx, celly, hitObject) {
                _this.aCotrller.onSceneTouch(cellx, celly, hitObject, true);
            };
            //长按
            this._sceneRoot.onLongTouch = function (hitObject) {
                if (hitObject)
                    _this.aCotrller.onLongSceneTouch(hitObject);
            };
