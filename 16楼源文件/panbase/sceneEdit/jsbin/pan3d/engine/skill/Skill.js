        ...
        var Skill = /** @class */ (function (_super) {
            ...
            Skill.prototype.getBloodTime = function () {
                if (this.skillVo) {
                    ...
                    return me.SkillVo.defaultBloodTime;
                }
            };
            Skill.prototype.play = function () {
                ...
                if (this.active && this.active instanceof me.Display3dMovie) {
            ...
            Skill.prototype.setKeyAry = function () {
                ...
                if (this.skillVo.types == me.SkillType.FixEffect) {
                    for (var i = 0; i < this.skillVo.keyAry.length; i++) {
                        var keySkill = new me.SkillFixEffect();
                ...
                else if (this.skillVo.types == me.SkillType.TrajectoryDynamicTarget || this.skillVo.types == me.SkillType.TrajectoryDynamicPoint) {
                    for (var i = 0; i < this.skillVo.keyAry.length; i++) {
                        var trajectory;
                        var tkv = (this.skillVo.keyAry[i]);
                        if (tkv.multype == 1) {
                            trajectory = new me.SkillMulTrajectory();
                        }
                        else {
                            trajectory = new me.SkillTrajectory();
                        }
            ...
            Skill.prototype.setendParticleRoation = function ($vect3d) {
                for (var i = 0; this.keyAry && i < this.keyAry.length; i++) {
                    if (this.keyAry[i] instanceof me.SkillTrajectory) {
            ...
            Skill.prototype.skillComplete = function () {
                me.SkillManager.getInstance().removeSkill(this);
                this.isDeath = true;
                if (this.completeFun) {
                    this.completeFun();
                }
            ...
            Skill.prototype.getKeyTarget = function () {
                ...
                for (var i = this.targetFlag; i < this.keyAry.length; i++) {
                    if (this.keyAry[i].time < this.time) {
                        ...
                        if (this.skillVo.types == me.SkillType.TrajectoryDynamicTarget || this.skillVo.types == me.SkillType.TrajectoryDynamicPoint) {
            ...
            Skill.prototype.getShockTarget = function () {
                ...
                for (var i = this.targetShockFlag; i < this.skillVo.shockAry.length; i++) {
                    if (this.skillVo.shockAry[i].time < this.time) {
                        //震动
                        me.ShockUtil.getInstance().shock(this.skillVo.shockAry[i].lasttime, this.skillVo.shockAry[i].amp);
                        i++;
            ...
            Skill.prototype.getSound = function () {
                ...
                if (this.skillVo.sound.frame < this.time) {
                    me.SoundManager.getInstance().playSkillSound(this.skillVo.sound.url);
                    this.soundPlay = true;
                }
            };
            ...
            Skill.prototype.configMulTrajectory = function ($activeList, $active, $target, $completeFun) {
                ...
                for (var i = 0; i < this.keyAry.length; i++) {
                    if (this.skillVo.types != me.SkillType.TrajectoryDynamicTarget) {
                        continue;
                    }
                    var skillTrajector = this.keyAry[i];
